...
static uint32_t extra_alloc(InternPool *pool, size_t n) {
    if (pool->extra_count + n > pool->extra_cap) {
        ...
        while (pool->extra_count + n > new_cap) new_cap *= 2;
...
static bool ip_key_eql(IpKey a, IpKey b) {
    ...
    switch (a.kind) {
    case IPK_PRIMITIVE_TYPE: return a.primitive_type == b.primitive_type;
    case IPK_RESERVED_VALUE: return a.reserved_value == b.reserved_value;
    case IPK_PTR_TYPE:
...
static IpKey ip_key_internal(InternPool *pool, IpIndex idx) {
    ...
    case IP_TAG_STRUCT_TYPE: {
        ...
        k.struct_type.field_types = (const IpIndex *)
            &pool->extra[data + 2 + k.struct_type.n_fields];
        return k;
    }
    case IP_TAG_ENUM_TYPE: {
        ...
        k.enum_type.variant_values = (const int64_t *)
            &pool->extra[data + 2 + k.enum_type.n_variants];
        return k;
    }
...
bool ip_is_type(InternPool *pool, IpIndex idx) {
    ...
    return t == IP_TAG_PRIMITIVE_TYPE ||
           t == IP_TAG_PTR_TYPE || t == IP_TAG_MANY_PTR_TYPE ||
           t == IP_TAG_SLICE_TYPE || t == IP_TAG_ARRAY_TYPE ||
           t == IP_TAG_OPTIONAL_TYPE || t == IP_TAG_FN_TYPE ||
           t == IP_TAG_STRUCT_TYPE || t == IP_TAG_ENUM_TYPE;
}

bool ip_is_value(InternPool *pool, IpIndex idx) {
    IpTag t = ip_tag(pool, idx);
    return t == IP_TAG_RESERVED_VALUE ||
           t == IP_TAG_INT_VALUE || t == IP_TAG_FLOAT_VALUE;
}
...
IpIndex ip_get(InternPool *pool, IpKey key) {
    ...
    IpTag tag;
    switch (key.kind) {
    case IPK_PTR_TYPE:       tag = IP_TAG_PTR_TYPE;       break;
    case IPK_MANY_PTR_TYPE:  tag = IP_TAG_MANY_PTR_TYPE;  break;
    case IPK_SLICE_TYPE:     tag = IP_TAG_SLICE_TYPE;     break;
    case IPK_ARRAY_TYPE:     tag = IP_TAG_ARRAY_TYPE;     break;
    case IPK_OPTIONAL_TYPE:  tag = IP_TAG_OPTIONAL_TYPE;  break;
    case IPK_FN_TYPE:        tag = IP_TAG_FN_TYPE;        break;
    case IPK_STRUCT_TYPE:    tag = IP_TAG_STRUCT_TYPE;    break;
    case IPK_ENUM_TYPE:      tag = IP_TAG_ENUM_TYPE;      break;
    case IPK_INT_VALUE:      tag = IP_TAG_INT_VALUE;      break;
    case IPK_FLOAT_VALUE:    tag = IP_TAG_FLOAT_VALUE;    break;
    case IPK_PRIMITIVE_TYPE:
    case IPK_RESERVED_VALUE:
...
static IpIndex ip_get_compound(InternPool *pool, IpKey key, IpTag tag) {
    ...
    while (1) {
        ...
        if (entry == 0) break; // empty — insert below
        ...
        if (entry_hh == hh) {
            ...
            if (pool->items_tag[idx] != IP_TAG_REMOVED) {
                ...
                if (ip_key_eql(key, existing)) return (IpIndex){idx};
            }
        }
        b = (b + 1) & mask;
    }
    ...
    case IP_TAG_SLICE_TYPE: {
        ...
        IpIndex elem; bool is_const;
        if (tag == IP_TAG_PTR_TYPE) {
            elem = key.ptr_type.elem; is_const = key.ptr_type.is_const;
        } else if (tag == IP_TAG_MANY_PTR_TYPE) {
            elem = key.many_ptr_type.elem; is_const = key.many_ptr_type.is_const;
        } else {
            elem = key.slice_type.elem; is_const = key.slice_type.is_const;
        }
...
void ip_dump_stats(InternPool *pool, FILE *out) {
    ...
    fprintf(out, "intern_pool: items=%zu (removed=%zu) extra=%zu/%zu buckets=%zu/%zu\n",
