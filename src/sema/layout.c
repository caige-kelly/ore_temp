...
static struct TypeLayout primitive_layout(struct Sema *s, struct Type *type) {
  switch (type->kind) {
  case TYPE_BOOL:
    return layout_complete(1, 1);
  case TYPE_U8:
  case TYPE_I8:
    return layout_complete(1, 1);
  case TYPE_U16:
  case TYPE_I16:
    return layout_complete(2, 2);
  case TYPE_U32:
  case TYPE_I32:
  case TYPE_F32:
  ...
  case TYPE_I64:
  case TYPE_F64:
    return layout_complete(8, 8);
  case TYPE_USIZE:
  case TYPE_ISIZE:
    return layout_complete(target_for(s).usize_size, target_for(s).usize_align);
  case TYPE_VOID:
  case TYPE_NORETURN:
    return layout_complete(0, 1);
  case TYPE_NIL:
    return layout_complete(target_for(s).pointer_size,
                           target_for(s).pointer_align);
  default:
...
static struct TypeLayout layout_from_field_decls(struct Sema *s,
                                                 struct Decl *owner) {
  ...
  for (size_t i = 0; i < scope->decls->count; i++) {
    ...
    if (!field)
      continue;
    if (field->kind != DECL_FIELD)
      continue;
    if (field->semantic_kind != SEM_VALUE)
      continue;
    ...
    if (!field_layout.complete)
      return layout_unknown();

    offset = align_up(offset, field_layout.align);
    offset += field_layout.size;
    if (field_layout.align > struct_align)
...
static struct TypeLayout compute_layout(struct Sema *s, struct Type *type) {
  ...
  case TYPE_SLICE:
    // {ptr, len}
    return layout_complete(target_for(s).pointer_size * 2,
                           target_for(s).pointer_align);
...
static const char *type_label(struct Sema *s, struct Type *type, char *buf,
                              size_t n) {
...
static struct Span span_for_type(struct Type *type, struct Span fallback) {
  if (type && type->decl)
    return type->decl->name.span;
  return fallback;
}

struct TypeLayout sema_layout_of_type_at(struct Sema *s, struct Type *type,
                                         struct Span ref_span) {
  if (!s || !type)
    return layout_unknown();
  if (sema_type_is_errorish(type))
    return layout_unknown();
