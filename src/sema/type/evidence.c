...
static struct EvidenceVector *alloc_vector(struct Sema *s) {
  if (!s || !s->arena)
    return NULL;
  struct EvidenceVector *ev =
      arena_alloc(s->arena, sizeof(struct EvidenceVector));
  if (!ev)
    return NULL;
...
void sema_evidence_record_call(struct Sema *s, struct Expr *call_expr) {
  ...
  if (!s->current_evidence || sema_evidence_len(s->current_evidence) == 0)
    return;
  struct EvidenceVector *snap = sema_evidence_clone(s, s->current_evidence);
  hashmap_put(&s->current_body->call_evidence, (uint64_t)(uintptr_t)call_expr,
              snap);
}

struct EvidenceVector *sema_evidence_at(struct Sema *s, struct Expr *expr) {
  if (!s || !expr || !s->current_body)
    return NULL;
  void *hit =
      hashmap_get(&s->current_body->call_evidence, (uint64_t)(uintptr_t)expr);
  if (hit)
    return (struct EvidenceVector *)hit;
  return s->current_body->entry_evidence;
}

struct EvidenceVector *sema_evidence_of_body(struct Sema *s,
