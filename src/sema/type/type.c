...
bool sema_value_fits_type(struct ConstValue v, struct Type *target) {
  ...
  if (v.kind == CONST_INT) {
    ...
    case TYPE_U64:
      return val >= 0; // int64_t max ≤ uint64_t max
    case TYPE_USIZE: {
    ...
    case TYPE_I64:
      return true; // val IS int64_t; always fits
    case TYPE_ISIZE:
      return true; // see usize note above
    ...
    case TYPE_COMPTIME_INT:
      return true; // staying comptime is always fine
    default:
      return true;
    }
  }
...
struct Type *sema_named_type(struct Sema *s, TypeKind kind, uint32_t name_id,
                             struct Decl *decl) {
...
struct Type *sema_handler_type(struct Sema *s, struct Decl *effect_decl,
                               struct Type *ret) {
...
struct Type *sema_numeric_join(struct Sema *s, struct Type *a, struct Type *b) {
  ...
  if (a->kind == b->kind)
    return a;
  if (a->kind == TYPE_COMPTIME_INT && b_int)
    return b;
  if (b->kind == TYPE_COMPTIME_INT && a_int)
    return a;
  if (a->kind == TYPE_COMPTIME_FLOAT && b_flt)
    return b;
  if (b->kind == TYPE_COMPTIME_FLOAT && a_flt)
    return a;
...
bool sema_type_is_integer(struct Type *type) {
  return type && (type->kind == TYPE_COMPTIME_INT || type->kind == TYPE_U8 ||
...
bool sema_type_is_float(struct Type *type) {
  return type && (type->kind == TYPE_F64 || type->kind == TYPE_F32 ||
...
bool sema_type_is_numeric(struct Type *type) {
  return type && (type->kind == TYPE_COMPTIME_INT || type->kind == TYPE_U8 ||
                  ...
                  type->kind == TYPE_I8 || type->kind == TYPE_I16 ||
                  type->kind == TYPE_I32 || type->kind == TYPE_I64 ||
                  type->kind == TYPE_ISIZE || type->kind == TYPE_F64 ||
                  type->kind == TYPE_F32 || type->kind == TYPE_COMPTIME_FLOAT);
}
...
struct Type *sema_primitive_type_for_name(struct Sema *s, uint32_t id) {
  ...
  if (!s)
    return NULL;
  struct Type *t =
      (struct Type *)hashmap_get(&s->primitive_types, (uint64_t)id);
...
static void append_text(char *buffer, size_t buffer_size, size_t *used,
                        const char *text) {
  if (!buffer || buffer_size == 0 || !used || !text)
    return;
  if (*used >= buffer_size)
    return;
...
const char *sema_type_display_name(struct Sema *s, struct Type *type,
  ...
  case TYPE_ARRAY: {
    ...
    if (type->array_length >= 0) {
      snprintf(buffer, buffer_size, "[%lld]%s", (long long)type->array_length,
               elem_name);
    } else {
  ...
  case TYPE_FUNCTION: {
    ...
    if (type->params) {
      for (size_t i = 0; i < type->params->count; i++) {
        ...
        append_text(buffer, buffer_size, &used,
                    sema_type_display_name(s, param ? *param : NULL, param_name,
                                           sizeof(param_name)));
      }
    }
    append_text(buffer, buffer_size, &used, ")");
    ...
    bool has_effect =
        sig && ((sig->terms && sig->terms->count > 0) || sig->is_open);
    if (has_effect) {
      append_text(buffer, buffer_size, &used, " <");
      bool first = true;
      if (sig->terms) {
  ...
  case TYPE_HANDLER: {
    const char *eff_name =
        (type->decl && type->decl->name.string_id)
            ? pool_get(s->pool, type->decl->name.string_id, 0)
            : "?";
    char ret_name[128];
    if (type->ret) {
      snprintf(
          buffer, buffer_size, "HandlerOf<%s, %s>", eff_name ? eff_name : "?",
          sema_type_display_name(s, type->ret, ret_name, sizeof(ret_name)));
    } else {
      snprintf(buffer, buffer_size, "HandlerOf<%s>", eff_name ? eff_name : "?");
    }
    return buffer;
  }
  default:
...
static bool effect_term_equal(struct EffectTerm *left,
                              struct EffectTerm *right) {
  if (left == right)
    return true;
  if (!left || !right)
    return false;
...
static bool effect_sig_equal(struct EffectSig *left, struct EffectSig *right) {
  ...
  if (left->terms == right->terms)
    return true;
  if (!left->terms || !right->terms ||
      left->terms->count != right->terms->count)
    return false;
  for (size_t i = 0; i < left->terms->count; i++) {
    struct EffectTerm *left_term = (struct EffectTerm *)vec_get(left->terms, i);
    struct EffectTerm *right_term =
        (struct EffectTerm *)vec_get(right->terms, i);
    if (!effect_term_equal(left_term, right_term))
      return false;
  }
  return true;
}
...
static bool effect_vec_equal(Vec *left, Vec *right) {
  ...
  for (size_t i = 0; i < left->count; i++) {
    ...
    if (!left_sig || !right_sig)
      return false;
    if (!effect_sig_equal(*left_sig, *right_sig))
      return false;
  }
  return true;
}
...
static bool type_vec_equal(Vec *left, Vec *right) {
  ...
  for (size_t i = 0; i < left->count; i++) {
    ...
    if (!left_type || !right_type)
      return false;
    if (!sema_type_equal(*left_type, *right_type))
      return false;
  }
  return true;
}
...
bool sema_type_assignable(struct Type *expected, struct Type *actual) {
  if (!expected || !actual)
    return true;
  if (sema_type_is_errorish(expected) || sema_type_is_errorish(actual))
    return true;
  ...
  if (actual->kind == TYPE_NORETURN)
    return true;
  if (expected->kind == TYPE_TYPE && sema_type_is_type_value(actual))
    return true;
  ...
  if (actual->kind == TYPE_NIL) {
    if (expected->is_optional)
      return true;
    return expected->kind == TYPE_NIL || expected->kind == TYPE_POINTER ||
           expected->kind == TYPE_SLICE;
  }
  ...
  if (actual->kind == TYPE_COMPTIME_INT && sema_type_is_numeric(expected))
    return true;
  if (actual->kind == TYPE_COMPTIME_FLOAT && sema_type_is_float(expected))
    return true;
