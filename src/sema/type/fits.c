...
static bool int_fits_unsigned(int64_t v, int bits, const char **lo,
  ...
  switch (bits) {
  case 8:  *lo = "0"; *hi = "255";        return u <= UINT8_MAX;
  case 16: *lo = "0"; *hi = "65535";      return u <= UINT16_MAX;
  case 32: *lo = "0"; *hi = "4294967295"; return u <= UINT32_MAX;
  case 64: *lo = "0"; *hi = "18446744073709551615"; return true;
  }
  return false;
}

static bool float_fits(double v, int bits, const char **lo,
                       const char **hi) {
  if (!isfinite(v)) {
    *lo = NULL; *hi = NULL;
    return false;
  ...
  double abs = v < 0 ? -v : v;
  switch (bits) {
  case 32: *lo = "-3.4e+38"; *hi = "3.4e+38";
           return abs <= (double)FLT_MAX;
  case 64: *lo = "-1.7e+308"; *hi = "1.7e+308";
           return abs <= DBL_MAX;
  }
  return false;
}
...
bool fits_in(struct ConstValue v, const struct Type *t,
             const char **out_min, const char **out_max) {
  ...
  if (v.kind == CONST_INT) {
    switch (t->kind) {
    case TY_I8:    ok = int_fits_signed(v.int_val,   8, &lo, &hi); break;
    case TY_I16:   ok = int_fits_signed(v.int_val,  16, &lo, &hi); break;
    case TY_I32:   ok = int_fits_signed(v.int_val,  32, &lo, &hi); break;
    case TY_I64:   ok = int_fits_signed(v.int_val,  64, &lo, &hi); break;
    case TY_ISIZE: ok = int_fits_signed(v.int_val,  64, &lo, &hi); break;
    case TY_U8:    ok = int_fits_unsigned(v.int_val, 8, &lo, &hi); break;
    case TY_U16:   ok = int_fits_unsigned(v.int_val,16, &lo, &hi); break;
    case TY_U32:   ok = int_fits_unsigned(v.int_val,32, &lo, &hi); break;
    case TY_U64:   ok = int_fits_unsigned(v.int_val,64, &lo, &hi); break;
    case TY_USIZE: ok = int_fits_unsigned(v.int_val,64, &lo, &hi); break;
    case TY_COMPTIME_INT: ok = true; break;
    default: ok = false; break;
    }
  } else if (v.kind == CONST_FLOAT) {
    switch (t->kind) {
    case TY_F32: ok = float_fits(v.float_val, 32, &lo, &hi); break;
    case TY_F64: ok = float_fits(v.float_val, 64, &lo, &hi); break;
    case TY_COMPTIME_FLOAT: ok = true; break;
    default: ok = false; break;
    }
  }
