...
static OreDefKind kind_for_def(struct DefInfo *info) {
  ...
  case SEM_VALUE:
    return ORE_DEF_VALUE;
  case SEM_TYPE:
    return ORE_DEF_TYPE;
  case SEM_EFFECT:
    return ORE_DEF_EFFECT;
  case SEM_MODULE:
    return ORE_DEF_MODULE;
  default:
    return ORE_DEF_VALUE;
  }
}
...
OreSpans ore_db_references(OreDb *db, uint32_t def_id) {
  ...
  for (size_t i = 0; i < nodes->count; i++) {
    ...
    if (db->sema.node_to_expr.entries == NULL)
      continue;
    if (!hashmap_contains(&db->sema.node_to_expr, (uint64_t)node_id->id))
      continue;
    struct Expr *e = (struct Expr *)hashmap_get(&db->sema.node_to_expr,
                                                (uint64_t)node_id->id);
    if (!e)
      continue;
...
void ore_cancel_token_destroy(OreCancelToken *tok) { free(tok); }
