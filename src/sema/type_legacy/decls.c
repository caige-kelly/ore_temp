...
static void try_fold_decl_value(struct Sema *s, struct Decl *decl,
  ...
  if (!decl->node->bind.value)
    return;
  if (info->value.kind != CONST_INVALID)
    return; // already folded
  if (info->fold_in_progress)
    return;
...
static bool decl_is_function_like(struct Decl *decl) {
  if (!decl || !decl->node)
    return false;
  if (expr_is_function_like(decl->node))
    return true;
...
static struct Param *find_param_decl(struct Decl *decl) {
  ...
  for (size_t i = 0; i < params->count; i++) {
    ...
    if (param && param->name.string_id == decl->name.string_id)
      return param;
  }
  return NULL;
}

static void assign_decl_type(struct Sema *s, struct Decl *decl,
                             struct Type *type) {
  if (!s || !decl || !type)
    return;
  ...
  if (type->effect_sig)
    info->effect_sig = type->effect_sig;
}
...
static void allocate_skeleton(struct Sema *s, struct Decl *decl) {
  ...
  case SEM_SCOPE_TOKEN:
    info->type =
        sema_named_type(s, TYPE_SCOPE_TOKEN, decl->name.string_id, decl);
    if (info->type)
      info->type->region_id = decl->scope_token_id;
    break;
  case SEM_EFFECT_ROW:
    info->type =
        sema_named_type(s, TYPE_EFFECT_ROW, decl->name.string_id, decl);
    break;
  case SEM_TYPE: {
...
static void walk_scope_decls(struct Sema *s, struct Scope *scope,
  ...
  if (scope->children) {
    for (size_t i = 0; i < scope->children->count; i++) {
      ...
      if (child_p && *child_p)
        walk_scope_decls(s, *child_p, visit);
    }
  }
}
...
static bool decl_signature_deferred(struct Sema *s, struct Decl *decl) {
  if (!decl || decl->semantic_kind != SEM_VALUE)
    return false;
  if (decl->kind == DECL_PARAM || decl->kind == DECL_PRIMITIVE ||
      decl->kind == DECL_IMPORT)
    return false;
  if (decl_is_function_like(decl))
    return false;
  if (decl->node && decl->node->kind == expr_Bind && decl->node->bind.type_ann)
    return false;
  return sema_decl_type(s, decl) == NULL;
}

static void resolve_struct_field(struct Sema *s, struct Decl *owner,
                                 struct FieldDef *field) {
  if (!field)
    return;

  struct Type *field_type =
      field->type ? sema_infer_type_expr(s, field->type) : s->unknown_type;
  struct Decl *field_decl = scope_lookup_member(
      owner ? owner->child_scope : NULL, field->name.string_id);
...
static void resolve_struct_signature(struct Sema *s, struct Decl *decl) {
  struct Expr *value = decl_bind_value(decl);
  if (!value || value->kind != expr_Struct || !value->struct_expr.members)
    return;
    ...
    if (member->kind == member_Field) {
      ...
      for (size_t j = 0; j < member->union_def.variants->count; j++) {
        resolve_struct_field(
            s, decl, (struct FieldDef *)vec_get(member->union_def.variants, j));
      }
    }
  }
}
...
static void resolve_enum_signature(struct Sema *s, struct Decl *decl) {
  ...
  for (size_t i = 0; i < value->enum_expr.variants->count; i++) {
    ...
    struct Decl *variant_decl =
        scope_lookup_member(decl->child_scope, variant->name.string_id);
    if (variant_decl && enum_type)
      assign_decl_type(s, variant_decl, enum_type);
  }
}

static void set_param_decl_type(struct Sema *s, struct Decl *owner,
                                struct Param *param, struct Type *type) {
  if (!owner || !owner->child_scope || !param)
    return;
  struct Decl *param_decl =
      scope_lookup_member(owner->child_scope, param->name.string_id);
  if (param_decl)
    assign_decl_type(s, param_decl, type);
}

static void resolve_function_signature_from_parts(struct Sema *s,
                                                  struct Decl *decl,
                                                  Vec *params,
  ...
  if (params && info->type->params && info->type->params->count == 0) {
    for (size_t i = 0; i < params->count; i++) {
      ...
      if (param)
        set_param_decl_type(s, decl, param, param_type);
    }
  }
  ...
  if (!info->type->ret || sema_type_is_errorish(info->type->ret)) {
    info->type->ret =
        ret_type ? sema_infer_type_expr(s, ret_type) : s->unknown_type;
  }
}
...
static void resolve_function_signature(struct Sema *s, struct Decl *decl) {
  ...
  if (decl->node->kind == expr_Lambda) {
    resolve_function_signature_from_parts(s, decl, decl->node->lambda.params,
                                          decl->node->lambda.ret_type,
                                          decl->node->lambda.effect);
    return;
  }
...
static void resolve_effect_signature(struct Sema *s, struct Decl *decl) {
  struct Expr *value = decl_bind_value(decl);
  if (!value || value->kind != expr_Effect || !value->effect_expr.operations)
    return;

  for (size_t i = 0; i < value->effect_expr.operations->count; i++) {
    struct Expr **op_p =
        (struct Expr **)vec_get(value->effect_expr.operations, i);
    struct Expr *op = op_p ? *op_p : NULL;
    if (!op || op->kind != expr_Bind)
      continue;

    struct Decl *op_decl =
        scope_lookup_member(decl->child_scope, op->bind.name.string_id);
    if (op_decl)
      sema_signature_of_decl(s, op_decl);
  }
}
...
static void report_decl_cycle(struct Sema *s, struct Decl *decl) {
  ...
  if (!info || info->type_query.state == QUERY_ERROR)
    return;
  const char *name =
      s->pool ? pool_get(s->pool, decl->name.string_id, 0) : NULL;
  sema_error(s, decl->name.span, "circular definition of '%s'",
             name ? name : "?");
...
static struct Type *compute_decl_signature(struct Sema *s, struct Decl *decl) {
  ...
  QueryBeginResult begin = sema_query_begin(
      s, &info->type_query, QUERY_TYPE_OF_DECL, decl, decl->name.span);
  switch (begin) {
  ...
  case QUERY_BEGIN_CYCLE:
    if (decl->semantic_kind == SEM_TYPE && info->type)
      return info->type;
  ...
  case SEM_VALUE:
    ...
    } else if (decl->node && decl->node->kind == expr_Bind &&
               decl->node->bind.type_ann) {
...
bool sema_collect_declarations(struct Sema *s) {
  ...
  for (size_t i = 0; i < modules->count; i++) {
    ...
    if (mod && mod->scope)
      walk_scope_decls(s, mod->scope, allocate_skeleton_visit);
  }
  ...
  for (size_t i = 0; i < modules->count; i++) {
    ...
    struct Module *mod = mod_p ? *mod_p : NULL;
    if (mod && mod->scope)
      walk_scope_decls(s, mod->scope, resolve_signature_visit);
  }
...
struct Type *sema_signature_of_decl(struct Sema *s, struct Decl *decl) {
  ...
  if (info->type && info->type_query.state == QUERY_DONE)
    return info->type;
