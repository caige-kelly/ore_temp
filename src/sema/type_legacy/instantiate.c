...
Vec *sema_decl_function_params(struct Decl *decl) {
  ...
  if (node->kind == expr_Lambda)
    return node->lambda.params;
  if (node->kind == expr_Ctl)
    return node->ctl.params;
  if (node->kind == expr_Bind && node->bind.value) {
    struct Expr *v = node->bind.value;
    if (v->kind == expr_Lambda)
      return v->lambda.params;
    if (v->kind == expr_Ctl)
      return v->ctl.params;
  }
  return NULL;
...
struct Expr *sema_decl_function_body(struct Decl *decl) {
  ...
  if (node->kind == expr_Ctl)
    return node->ctl.body;
  if (node->kind == expr_Bind && node->bind.value) {
    struct Expr *v = node->bind.value;
    if (v->kind == expr_Lambda)
      return v->lambda.body;
    if (v->kind == expr_Ctl)
      return v->ctl.body;
  }
  return NULL;
...
struct Expr *sema_decl_function_ret_type_expr(struct Decl *decl) {
  ...
  struct Expr *node = decl->node;
  if (node->kind == expr_Lambda)
    return node->lambda.ret_type;
  if (node->kind == expr_Ctl)
    return node->ctl.ret_type;
  if (node->kind == expr_Bind && node->bind.value) {
    struct Expr *v = node->bind.value;
    if (v->kind == expr_Lambda)
      return v->lambda.ret_type;
    if (v->kind == expr_Ctl)
      return v->ctl.ret_type;
  }
  return NULL;
}
...
struct ComptimeArgTuple sema_arg_tuple_new(struct Sema *s) {
  ...
  if (s && s->arena)
    t.values = vec_new_in(s->arena, sizeof(struct ConstValue));
  return t;
}

void sema_arg_tuple_push(struct ComptimeArgTuple *tuple,
                         struct ConstValue value) {
  if (!tuple || !tuple->values)
    return;
  vec_push(tuple->values, &value);
}

bool sema_arg_tuple_equal(const struct ComptimeArgTuple *a,
                          const struct ComptimeArgTuple *b) {
  if (!a || !b)
    return a == b;
  Vec *av = a->values;
  Vec *bv = b->values;
  ...
  for (size_t i = 0; i < av->count; i++) {
    ...
    switch (ax->kind) {
    case CONST_INT:
      if (ax->int_val != bx->int_val)
        return false;
      break;
    case CONST_FLOAT:
      if (ax->float_val != bx->float_val)
        return false;
      break;
    case CONST_BOOL:
      if (ax->bool_val != bx->bool_val)
        return false;
      break;
    case CONST_TYPE:
      if (!sema_type_equal(ax->type_val, bx->type_val))
        return false;
      break;
    case CONST_STRING:
      if (ax->string_id != bx->string_id)
        return false;
      break;
    case CONST_FUNCTION:
      if (ax->fn_decl != bx->fn_decl)
        return false;
      break;
    case CONST_STRUCT: {
    ...
    case CONST_ARRAY:
      if (ax->array_val->elements != bx->array_val->elements)
        return false;
      break;
    case CONST_VOID:
      break;
...
static Vec *instantiation_bucket(struct Sema *s, struct Decl *generic,
                                 bool create) {
  Vec *bucket = (Vec *)hashmap_get(&s->instantiation_buckets,
                                   (uint64_t)(uintptr_t)generic);
  if (bucket || !create)
    return bucket;
...
static struct Instantiation *
find_existing(struct Sema *s, struct Decl *generic,
  ...
  for (size_t i = 0; i < bucket->count; i++) {
    ...
    if (!inst)
      continue;
    if (sema_arg_tuple_equal(&inst->args, args))
      return inst;
  }
  return NULL;
}
...
static struct Decl *find_param_decl_in(struct Scope *scope, uint32_t name_id) {
  if (!scope)
    return NULL;
  struct Decl *d =
      (struct Decl *)hashmap_get(&scope->name_index, (uint64_t)name_id);
...
static void bind_comptime_args(struct Sema *s, struct ComptimeEnv *env,
  ...
  for (size_t i = 0; i < params->count && arg_idx < args->values->count; i++) {
    ...
    struct Decl *pd =
        find_param_decl_in(generic->child_scope, p->name.string_id);
    if (pd)
      sema_comptime_env_bind(s, env, pd, *v);
    arg_idx++;
  }
}

// Build the specialized function type by re-evaluating each (non-comptime)
// param's type annotation and the return type with the instantiation env
// active.
static struct Type *build_specialized_type(struct Sema *s,
                                           struct Decl *generic) {
  ...
  if (params) {
    for (size_t i = 0; i < params->count; i++) {
      ...
      if (p->kind != PARAM_RUNTIME)
        continue; // erased — not part of the runtime signature
      struct Type *pt =
          p->type_ann ? sema_infer_type_expr(s, p->type_ann) : s->unknown_type;
...
struct Instantiation *sema_instantiate_decl(struct Sema *s,
                                            struct Decl *generic,
  ...
  struct Instantiation *inst =
      arena_alloc(s->arena, sizeof(struct Instantiation));
  if (!inst)
    return NULL;
  ...
  if (bucket)
    vec_push(bucket, &inst);

  QueryBeginResult begin = sema_query_begin(
      s, &inst->query, QUERY_INSTANTIATE_DECL, inst, generic->name.span);
  ...
  inst->specialized_sig =
      inst->specialized_type ? inst->specialized_type->effect_sig : NULL;
