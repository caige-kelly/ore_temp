...
static void resolver_error(struct Resolver *r, struct Span span,
                           const char *fmt, ...) {
  char msg[512];
  va_list ap;
  va_start(ap, fmt);
...
struct Scope *scope_new(struct Resolver *r, ScopeKind kind,
                        struct Scope *parent) {
...
static struct Decl *scope_lookup_with_overlays(struct Resolver *r,
                                               uint32_t string_id) {
...
static struct Decl *decl_new(struct Resolver *r, struct Scope *owner,
  ...
  if (existing) {
    ...
    resolver_error(r, name->span, "'%s' is already defined in this scope",
                   nm ? nm : "?");
    return NULL;
  }
  ...
  if (r->handler_body_depth > 0 && r->compiler) {
    ...
    hashmap_put(&r->compiler->handler_impl_decls, (uint64_t)(uintptr_t)d,
                (void *)d);
  }
  return d;
}

static SemanticKind semantic_kind_for_decl_value(struct Expr *value,
                                                 DeclKind decl_kind) {
  if (decl_kind == DECL_IMPORT)
    return SEM_MODULE;
  if (!value)
    return SEM_VALUE;
  switch (value->kind) {
...
static struct Decl *ensure_implicit_decl(struct Resolver *r,
                                         struct Scope *owner, DeclKind kind,
                                         SemanticKind semantic_kind,
                                         struct Identifier *id,
                                         struct Expr *node) {
  if (!id || id->string_id == 0)
    return NULL;
...
static bool is_import_expr(struct Resolver *r, struct Expr *expr) {
  if (!expr || expr->kind != expr_Builtin)
    return false;
...
static struct Expr *import_path_arg(struct Resolver *r, struct Expr *expr) {
  ...
  if (!expr->builtin.args || expr->builtin.args->count != 1) {
    resolver_error(r, expr->span,
                   "@import expects exactly one string literal path");
    return NULL;
  }

  struct Expr **arg = (struct Expr **)vec_get(expr->builtin.args, 0);
  if (!arg || !*arg || (*arg)->kind != expr_Lit ||
      (*arg)->lit.kind != lit_String) {
...
static struct Module *module_find(struct Resolver *r, uint32_t path_id) {
  if (!r->compiler || !r->compiler->module_map)
    return NULL;
  return hashmap_get(r->compiler->module_map, path_id);
}

static struct Module *module_new(struct Resolver *r, uint32_t path_id,
                                 Vec *ast) {
...
static void report_import_cycle(struct Resolver *r, struct Module *repeated,
                                struct Span span) {
  char msg[256];
  const char *path = pool_get(r->pool, repeated->path_id, 0);
  snprintf(msg, sizeof(msg), "circular import involving %s",
           path ? path : "<unknown>");
...
static struct Module *load_imported_module(struct Resolver *r,
                                           struct Expr *import_expr) {
  ...
  const char *importer_path =
      r->current_module ? pool_get(r->pool, r->current_module->path_id, 0)
  ...
  char *canonical_path =
      ore_resolve_import_path_in(path_arena, importer_path, raw_path);
  if (!canonical_path) {
    if (path_arena)
      arena_reset_to(path_arena, path_mark);
    resolver_error(r, import_expr->span, "could not resolve import path '%s'",
                   raw_path ? raw_path : "?");
    return NULL;
  }
  if (path_arena)
  ...
  uint32_t path_id =
      pool_intern(r->pool, canonical_path, strlen(canonical_path));
  struct Module *mod = module_find(r, path_id);
  if (!mod) {
    Vec *ast = ore_parse_file(r->compiler, canonical_path,
                              r->compiler->next_file_id++);
    if (!ast) {
  ...
  if (!resolve_module(r, mod))
    return NULL;
  return mod;
}

static bool resolve_import_binding(struct Resolver *r, struct Decl *decl,
                                   struct Expr *bind_expr) {
  if (!decl || !bind_expr || bind_expr->kind != expr_Bind)
    return false;
  struct Expr *import_expr = bind_expr->bind.value;
  if (!is_import_expr(r, import_expr))
    return false;
...
void register_primitives(struct Resolver *r) {
  static const char *prims[] = {
      ...
      "u32",      "u64",  "usize",   "f32",   "f64",   "bool",  "void",
      "noreturn", "type", "anytype", "Scope", "true",  "false", "nil",
  };
...
static void EffectExpr_seed_decls(struct Resolver *r, struct Scope *scope,
                                  struct EffectExpr *eff) {
  if (!eff || !eff->operations)
    return;
...
static void StructExpr_seed_decls(struct Resolver *r, struct Scope *scope,
                                  struct StructExpr *st) {
  if (!st || !st->members)
    return;
...
static void EnumExpr_seed_decls(struct Resolver *r, struct Scope *scope,
                                struct EnumExpr *en) {
  if (!en || !en->variants)
    return;
...
void collect_decl(struct Resolver *r, struct Expr *expr) {
  ...
  if (expr->kind == expr_DestructureBind) {
    ...
    if (!pattern || pattern->kind != expr_Product)
      return;
    ...
    for (size_t i = 0; i < prod->Fields->count; i++) {
      ...
      if (!f || !f->value || f->value->kind != expr_Ident)
        continue;
      struct Decl *d =
          decl_new(r, r->current, DECL_USER, &f->value->ident, expr);
      if (!d)
        continue;
...
static void resolve_lambda_into(struct Resolver *r, struct Expr *lambda,
...
static struct Scope *lambda_fn_scope_or_new(struct Resolver *r,
                                            struct Decl *owning_decl) {
  struct Scope *fn_scope = owning_decl ? owning_decl->child_scope : NULL;
  if (!fn_scope) {
    fn_scope = scope_new(r, SCOPE_FUNCTION, r->current);
    if (owning_decl)
      owning_decl->child_scope = fn_scope;
  }
  return fn_scope;
}
...
static bool decl_is_scoped_effect(struct Decl *d) {
  if (!d || d->semantic_kind != SEM_EFFECT || !d->node)
    return false;
  if (d->node->kind != expr_Bind || !d->node->bind.value)
    return false;
  struct Expr *value = d->node->bind.value;
  return value->kind == expr_Effect && value->effect_expr.is_scoped;
}

static void declare_effect_annotation_params(struct Resolver *r,
                                             struct Scope *owner,
                                             struct Expr *expr,
                                             struct Expr *node) {
  if (!expr || !owner)
...
static void validate_effect_annotation_expr(struct Resolver *r,
                                            struct Expr *expr) {
  if (!expr)
    return;
  ...
  case expr_Field: {
    ...
    if (d && d->semantic_kind != SEM_EFFECT &&
      ...
      resolver_error(r, expr->span, "'%s' is not an effect", nm ? nm : "?");
    }
    return;
  }
  case expr_Call: {
...
static void register_effect_ops_in_scope(struct Resolver *r,
                                         struct Scope *scope,
                                         struct Decl *eff) {
  if (!r || !scope || !eff || !eff->child_scope)
    return;
  ...
  for (size_t i = 0; i < decls->count; i++) {
    ...
    if (existing) {
      ...
      resolver_error(r, op->name.span, "'%s' is already defined in this scope",
                     nm ? nm : "?");
      continue;
    }
...
static void inject_ops_from_annotation(struct Resolver *r, struct Scope *scope,
                                       struct Expr *annotation) {
  if (!r || !scope || !annotation)
    return;
  ...
  while (work->count > 0) {
    ...
    switch (e->kind) {
    case expr_EffectRow:
      if (e->effect_row.head)
        vec_push(work, &e->effect_row.head);
      break;
    case expr_Bin:
      if (e->bin.Left)
        vec_push(work, &e->bin.Left);
      if (e->bin.Right)
        vec_push(work, &e->bin.Right);
      break;
    case expr_Call:
      if (e->call.callee)
        vec_push(work, &e->call.callee);
      break;
    case expr_Field:
      if (e->field.object)
        vec_push(work, &e->field.object);
      break;
    case expr_Lambda:
      if (e->lambda.effect)
        vec_push(work, &e->lambda.effect);
      if (e->lambda.params) {
        for (size_t i = 0; i < e->lambda.params->count; i++) {
          struct Param *p = (struct Param *)vec_get(e->lambda.params, i);
          if (p && p->type_ann)
            vec_push(work, &p->type_ann);
        }
      }
      if (e->lambda.ret_type)
        vec_push(work, &e->lambda.ret_type);
      break;
    default:
      break;
    }
...
static bool is_handler_lifecycle_bind(struct Resolver *r, uint32_t string_id) {
  return string_id == r->initially_name_id || string_id == r->finally_name_id;
}
...
static void handler_op_name_set(struct HandlerExpr *h, HashMap *out) {
  ...
  for (size_t i = 0; i < h->operations->count; i++) {
    ...
    if (op)
      hashmap_put(out, (uint64_t)op->name.string_id, (void *)1);
  }
}
...
static void effect_op_name_set(struct Decl *eff, HashMap *out) {
  if (!eff || !eff->child_scope || !eff->child_scope->decls)
    return;
  Vec *decls = eff->child_scope->decls;
  for (size_t i = 0; i < decls->count; i++) {
    struct Decl **dp = (struct Decl **)vec_get(decls, i);
...
static bool op_name_sets_equal(HashMap *a, HashMap *b) {
  ...
  for (size_t i = 0; i < a->capacity; i++) {
    if (!a->entries[i].occupied)
      continue;
    if (!hashmap_contains(b, a->entries[i].key))
      return false;
  }
  return true;
}
...
static void format_op_name_set(StringPool *pool, HashMap *set, char *buf,
                               size_t cap) {
  if (cap == 0)
    return;
  buf[0] = '\0';
...
static struct Decl *resolve_handler_effect(struct Resolver *r,
  ...
  for (struct Scope *cur = r->current; cur; cur = cur->parent) {
    ...
    for (size_t i = 0; i < cur->decls->count; i++) {
      ...
      if (!op_name_sets_equal(&want, &got))
        continue;

      if (!match)
        match = d;
      else if (match != d && !second_match)
        second_match = d;
    }
  }
  ...
  for (struct Scope *cur = r->current; cur; cur = cur->parent) {
    ...
    for (size_t i = 0; i < cur->decls->count; i++) {
      ...
      for (size_t k = 0; k < want.capacity; k++) {
        if (!want.entries[k].occupied)
          continue;
        if (hashmap_contains(&got, want.entries[k].key))
          overlap++;
      }
  ...
  if (nearest) {
    ...
    resolver_error(r, span,
                   "handler doesn't match any effect in scope (ops: %s); "
                   "nearest is '%s' (ops: %s)",
                   ops, nm ? nm : "?", eff_ops);
  } else {
    resolver_error(r, span,
...
static void resolve_handler_payload(struct Resolver *r, struct HandlerExpr *h,
                                    struct Span span) {
  ...
  if (h->initially_clause) {
    if ((h->initially_clause->kind == expr_Block &&
         h->initially_clause->block.stmts) ||
        h->initially_clause->kind == expr_Bind) {
      ...
      resolver_error(r, h->initially_clause->span,
                     "Initially block must be an expression block");
    }
  }
...
static void resolve_expr_inner(struct Resolver *r, struct Expr *expr) {
  ...
  case expr_Ident: {
    ...
    if (d) {
      ...
      if ((d->semantic_kind == SEM_SCOPE_TOKEN ||
        ...
        const char *nm = pool_get(r->pool, d->name.string_id, 0);
        resolver_error(r, expr->span,
                       "'%s' is a comptime effect token and cannot be used as "
                       "a runtime value",
                       nm ? nm : "?");
      }
    ...
    if (expr->call.args) {
      ...
      for (size_t i = 0; i < expr->call.args->count; i++) {
        ...
        while (callee_params && pi < callee_params->count) {
          ...
          if (p && p->kind == PARAM_INFERRED_COMPTIME) {
            pi++;
            continue;
          }
          break;
        }
        if (arg && arg->kind == expr_Lambda && callee_params &&
            pi < callee_params->count) {
          struct Param *p = (struct Param *)vec_get(callee_params, pi);
          if (p && p->type_ann && p->type_ann->kind == expr_Lambda) {
            ...
            if (sig->lambda.params && arg->lambda.params) {
              ...
              for (size_t k = 0; k < n; k++) {
                struct Param *ap =
                    (struct Param *)vec_get(arg->lambda.params, k);
                struct Param *sp =
                    (struct Param *)vec_get(sig->lambda.params, k);
  ...
  case expr_Builtin:
    ...
    if (is_import_expr(r, expr)) {
      resolver_error(r, expr->span,
                     "@import must be used as a module-level alias, e.g. name "
                     ":: @import(\"file.ore\")");
      return;
    }
  ...
  case expr_EffectRow:
    ...
    if (expr->effect_row.row.string_id != 0 &&
      ...
      expr->effect_row.row.resolved =
          scope_lookup(r->current, expr->effect_row.row.string_id);
    }
    return;
  ...
  case expr_If: {
    ...
    if (expr->if_expr.capture.string_id != 0) {
      ...
      struct Scope *saved = r->current;
      r->current = cap_scope;
      struct Decl *cap_decl =
          decl_new(r, cap_scope, DECL_USER, &expr->if_expr.capture, expr);
      if (cap_decl)
        cap_decl->semantic_kind = SEM_VALUE;
  ...
  case expr_Bind: {
    ...
    if (is_local && expr->bind.kind == bind_Const) {
      local_decl = decl_new(r, r->current, DECL_USER, &expr->bind.name, expr);
      if (local_decl) {
        local_decl->is_comptime = true;
        local_decl->is_export = false;
        local_decl->semantic_kind =
            semantic_kind_for_decl_value(expr->bind.value, DECL_USER);
      }
    }
    ...
    if (expr->bind.value && expr->bind.value->kind == expr_Lambda) {
      ...
      if (!type_scope) {
        ...
        if (owning_decl)
          owning_decl->child_scope = type_scope;
      }
      struct Scope *saved = r->current;
    ...
    if (is_local && expr->bind.kind != bind_Const) {
      struct Decl *d =
          decl_new(r, r->current, DECL_USER, &expr->bind.name, expr);
      if (d) {
        d->is_comptime = false;
        d->is_export = false;
        d->semantic_kind =
  ...
  case expr_DestructureBind: {
    ...
    if (is_local && expr->destructure.pattern &&
      ...
      if (prod->Fields) {
        for (size_t i = 0; i < prod->Fields->count; i++) {
          struct ProductField *f =
              (struct ProductField *)vec_get(prod->Fields, i);
          if (!f || !f->value)
            continue;
          if (f->value->kind != expr_Ident)
            continue;
          struct Decl *dd =
              decl_new(r, r->current, DECL_USER, &f->value->ident, expr);
          if (dd) {
  ...
  case expr_Field:
    ...
    resolve_expr(r, expr->field.object);
    {
      ...
      if (object_decl && object_decl->child_scope) {
        struct Decl *field_decl = scope_lookup_local(
            object_decl->child_scope, expr->field.field.string_id);
        if (field_decl) {
          ...
          const char *obj_nm =
              pool_get(r->pool, object_decl->name.string_id, 0);
          const char *field_nm =
              pool_get(r->pool, expr->field.field.string_id, 0);
          const char *kind = "namespace";
          if (object_decl->kind == DECL_IMPORT) {
            kind = "module";
          } else if (object_decl->node &&
                     object_decl->node->kind == expr_Bind) {
  ...
  case expr_Loop: {
    ...
    if (expr->loop_expr.capture.string_id != 0) {
      struct Decl *cap_decl =
          decl_new(r, loop_scope, DECL_USER, &expr->loop_expr.capture, expr);
      if (cap_decl)
        cap_decl->semantic_kind = SEM_VALUE;
    }
...
static void validate_identifier_reference(struct Resolver *r,
                                          struct Identifier id) {
  if (id.string_id == 0 || id.resolved)
    return;
  const char *nm = pool_get(r->pool, id.string_id, 0);
  resolver_error(r, id.span, "'%s' is not defined in any accessible scope",
                 nm ? nm : "?");
}

static void validate_effect_row_reference(struct Resolver *r,
                                          struct Identifier id) {
  if (id.string_id == 0 || id.resolved)
    return;
  const char *nm = pool_get(r->pool, id.string_id, 0);
  resolver_error(r, id.span, "'%s' must be an effect-row variable in scope",
                 nm ? nm : "?");
}

static void validate_field_def_identifiers(struct Resolver *r,
                                           struct FieldDef *f) {
  if (!f)
    return;
...
static void validate_struct_member_identifiers(struct Resolver *r,
                                               struct StructMember *m) {
  if (!m)
    return;
  if (m->kind == member_Field) {
    ...
    for (size_t i = 0; i < m->union_def.variants->count; i++) {
      validate_field_def_identifiers(
          r, (struct FieldDef *)vec_get(m->union_def.variants, i));
    }
  }
}
...
static void validate_handler_payload(struct Resolver *r,
                                     struct HandlerExpr *h) {
  if (!h)
    return;
  if (h->target)
    ...
    for (size_t i = 0; i < h->operations->count; i++) {
      ...
      if (op->params) {
        for (size_t j = 0; j < op->params->count; j++) {
          struct Param *p = (struct Param *)vec_get(op->params, j);
          if (p)
            validate_expr_identifiers(r, p->type_ann);
        }
      }
...
static void validate_expr_identifiers(struct Resolver *r, struct Expr *expr) {
  ...
  case expr_Block: {
    ...
    for (size_t i = 0; i < stmts->count; i++) {
      struct Expr **st = (struct Expr **)vec_get(stmts, i);
      if (st)
        validate_expr_identifiers(r, *st);
    }
    return;
  }
  case expr_Product:
    ...
    if (expr->product.Fields) {
      for (size_t i = 0; i < expr->product.Fields->count; i++) {
        struct ProductField *f =
            (struct ProductField *)vec_get(expr->product.Fields, i);
        if (f)
          validate_expr_identifiers(r, f->value);
      }
    }
    return;
  case expr_Bind:
  ...
  case expr_Struct:
    if (expr->struct_expr.members) {
      for (size_t i = 0; i < expr->struct_expr.members->count; i++) {
        validate_struct_member_identifiers(
            r, (struct StructMember *)vec_get(expr->struct_expr.members, i));
      }
    }
    return;
  case expr_Enum:
    if (expr->enum_expr.variants) {
      for (size_t i = 0; i < expr->enum_expr.variants->count; i++) {
        struct EnumVariant *v =
            (struct EnumVariant *)vec_get(expr->enum_expr.variants, i);
        if (v)
          validate_expr_identifiers(r, v->explicit_value);
      }
    }
    return;
  case expr_EnumRef:
    ...
    if (expr->effect_expr.operations) {
      for (size_t i = 0; i < expr->effect_expr.operations->count; i++) {
        struct Expr **op =
            (struct Expr **)vec_get(expr->effect_expr.operations, i);
        if (op)
          validate_expr_identifiers(r, *op);
      }
    }
    return;
  case expr_Return:
...
static void validate_resolved_identifiers(struct Resolver *r) {
  ...
  for (size_t i = 0; i < r->ast->count; i++) {
    ...
    if (expr)
      validate_expr_identifiers(r, *expr);
  }
}
...
static void dump_scope(struct Resolver *r, struct Scope *s, int depth) {
  ...
  for (size_t i = 0; i < s->decls->count; i++) {
    ...
    const char *nm = pool_get(r->pool, (*d)->name.string_id, 0);
    bool handler_impl =
        r->compiler && hashmap_contains(&r->compiler->handler_impl_decls,
                                        (uint64_t)(uintptr_t)*d);
    bool is_pub = false;
    if ((*d)->node) {
      if ((*d)->node->kind == expr_Bind)
        is_pub = (*d)->node->bind.is_pub;
      else if ((*d)->node->kind == expr_DestructureBind)
        is_pub = (*d)->node->destructure.is_pub;
    }
    printf("decl %s %s <%s>%s%s%s%s%s%s%s", decl_kind_str((*d)->kind),
           nm ? nm : "?", semantic_kind_str((*d)->semantic_kind),
           (*d)->is_comptime ? " [comptime]" : "", is_pub ? " [pub]" : "",
           (*d)->is_export ? " [export]" : "",
...
static void tally_expr(struct Resolver *r, struct Expr *expr,
                       struct RefStats *s);

static void tally_field_def(struct Resolver *r, struct FieldDef *f,
                            struct RefStats *s) {
  if (!f)
    return;
...
static void tally_struct_member(struct Resolver *r, struct StructMember *m,
                                struct RefStats *s) {
  if (!m)
    return;
  if (m->kind == member_Field) {
    ...
    if (m->union_def.variants) {
      for (size_t i = 0; i < m->union_def.variants->count; i++) {
        tally_field_def(r, (struct FieldDef *)vec_get(m->union_def.variants, i),
                        s);
      }
    }
  }
...
static void tally_handler_payload(struct Resolver *r, struct HandlerExpr *h,
                                  struct RefStats *s) {
  if (!h)
    return;
  if (h->target)
...
static void tally_expr(struct Resolver *r, struct Expr *expr,
  ...
  case expr_Ident:
    ...
    return;
  case expr_Bin:
    tally_expr(r, expr->bin.Left, s);
    tally_expr(r, expr->bin.Right, s);
    return;
  case expr_Assign:
    tally_expr(r, expr->assign.target, s);
    tally_expr(r, expr->assign.value, s);
    return;
  case expr_Unary:
    tally_expr(r, expr->unary.operand, s);
    return;
  case expr_Call:
    ...
    if (expr->enum_expr.variants)
      for (size_t i = 0; i < expr->enum_expr.variants->count; i++) {
        struct EnumVariant *v =
            (struct EnumVariant *)vec_get(expr->enum_expr.variants, i);
        if (v)
          tally_expr(r, v->explicit_value, s);
      }
    return;
  case expr_EnumRef:
    return;
  case expr_Effect:
    ...
    return;
  case expr_Return:
    tally_expr(r, expr->return_expr.value, s);
    return;
  case expr_Defer:
    tally_expr(r, expr->defer_expr.value, s);
    return;
  case expr_ArrayType:
  ...
  case expr_ArrayLit:
    ...
    return;
  case expr_SliceType:
    tally_expr(r, expr->slice_type.elem, s);
    return;
  case expr_ManyPtrType:
    tally_expr(r, expr->many_ptr_type.elem, s);
    return;
  }
}
...
void dump_resolution(struct Resolver *r) {
  ...
  stats.resolved_sample =
      vec_new_in(stats_arena, sizeof(struct ResolvedSample));
  stats.resolved_cap_for_display = 20;
  for (size_t i = 0; i < r->ast->count; i++) {
    struct Expr **e = (struct Expr **)vec_get(r->ast, i);
    if (e && *e)
      tally_expr(r, *e, &stats);
  }
  ...
  if (stats.resolved_sample->count > 0) {
    ...
    for (size_t i = 0; i < stats.resolved_sample->count; i++) {
      struct ResolvedSample *rs =
          (struct ResolvedSample *)vec_get(stats.resolved_sample, i);
      ...
      printf("    %s @ %d:%d -> %s %s (decl @ %d:%d)\n", use_nm ? use_nm : "?",
             ...
             decl_kind_str(rs->decl->kind), decl_nm ? decl_nm : "?",
...
struct Resolver resolver_new(struct Compiler *compiler, Vec *ast) {
  ...
  r.root_path_id = compiler->root_path
                       ? pool_intern(&compiler->pool, compiler->root_path,
                                     strlen(compiler->root_path))
                       : 0;
...
static bool resolve_module(struct Resolver *r, struct Module *mod) {
  ...
  size_t error_count_after =
      r->diags ? r->diags->error_count : error_count_before;
