...
GreenNode *green_node_alloc(SyntaxKind kind, const GreenElement *children,
                            uint32_t num_children) {
    size_t bytes = sizeof(GreenNode) + (size_t)num_children * sizeof(GreenElement);
...
GreenElement green_node_child(const GreenNode *n, uint32_t i) {
    if (i >= n->num_children) {
        return (GreenElement){.kind = GREEN_ELEM_NODE, .rel_offset = 0,
                              .node = NULL};
    }
    return n->children[i];
}
...
GreenNode *green_node_replace_child(const GreenNode *n, uint32_t idx,
    ...
    GreenElement *buf = (GreenElement *)malloc(
        (size_t)n->num_children * sizeof(GreenElement));
    if (!buf) abort();
    for (uint32_t i = 0; i < n->num_children; i++) {
...
GreenNode *green_node_splice_children(const GreenNode *n,
    ...
    assert((count == 0 || replace != NULL) && "splice_children: NULL replace + non-zero count");
    ...
    if (new_count > 0) {
        ...
        if (!buf) abort();
        uint32_t out_idx = 0;
        for (uint32_t i = 0; i < from; i++)               buf[out_idx++] = n->children[i];
        for (uint32_t i = 0; i < count; i++)              buf[out_idx++] = replace[i];
        for (uint32_t i = to; i < n->num_children; i++)   buf[out_idx++] = n->children[i];
