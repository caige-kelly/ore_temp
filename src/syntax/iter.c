...
//       else if current has a next sibling: move to it; emit Enter(sibling);
//       descending = true. else: move to parent; emit Leave(parent); descending
//       = false.
//
...
static SyntaxElement retain_elem(SyntaxElement e) {
  if (e.kind == SYNTAX_ELEM_NODE && e.node)
    syntax_node_retain(e.node);
  else if (e.kind == SYNTAX_ELEM_TOKEN && e.token)
    syntax_token_retain(e.token);
  return e;
}
...
SyntaxNode *syntax_descendants_next(SyntaxDescendants *it) {
  for (;;) {
    ...
    if (syntax_walk_event_is_none(ev))
      return NULL;
...
void syntax_siblings_init(SyntaxSiblings *it, SyntaxNode *start,
                          SyntaxDirection dir) {
  it->cur = start;
  if (start)
    syntax_node_retain(start);
...
static void siblings_elem_init_common(SyntaxSiblingsElem *it,
                                      SyntaxElement start,
                                      SyntaxDirection dir) {
  it->cur = start;
  if (start.kind == SYNTAX_ELEM_NODE && start.node)
    syntax_node_retain(start.node);
  else if (start.kind == SYNTAX_ELEM_TOKEN && start.token)
    syntax_token_retain(start.token);
  it->dir = dir;
  it->emitted_first = false;
}

void syntax_siblings_elem_init_node(SyntaxSiblingsElem *it, SyntaxNode *start,
                                    SyntaxDirection dir) {
  siblings_elem_init_common(
      it, (SyntaxElement){.kind = SYNTAX_ELEM_NODE, .node = start}, dir);
}

void syntax_siblings_elem_init_token(SyntaxSiblingsElem *it, SyntaxToken *start,
                                     SyntaxDirection dir) {
  siblings_elem_init_common(
      it, (SyntaxElement){.kind = SYNTAX_ELEM_TOKEN, .token = start}, dir);
}

static SyntaxElement elem_next_sibling_or_token(SyntaxElement e,
                                                SyntaxDirection dir) {
  if (syntax_element_is_none(e))
    return SYNTAX_ELEMENT_NONE;
  if (e.kind == SYNTAX_ELEM_NODE) {
    return (dir == SYNTAX_DIR_NEXT) ? syntax_node_next_sibling_or_token(e.node)
                                    : syntax_node_prev_sibling_or_token(e.node);
  }
  return (dir == SYNTAX_DIR_NEXT) ? syntax_token_next_sibling_or_token(e.token)
                                  : syntax_token_prev_sibling_or_token(e.token);
}

SyntaxElement syntax_siblings_elem_next(SyntaxSiblingsElem *it) {
  if (syntax_element_is_none(it->cur))
    return SYNTAX_ELEMENT_NONE;
  if (!it->emitted_first) {
    it->emitted_first = true;
