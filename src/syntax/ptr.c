...
static uint32_t find_child_containing(const SyntaxNode *parent, TextRange target) {
    ...
    while (lo < hi) {
        ...
        if (target.start >= child_end) {
            ...
            if (text_range_contains(child, target)) return mid;
...
SyntaxNode *syntax_node_ptr_resolve(SyntaxNodePtr ptr, SyntaxNode *root) {
    ...
    for (;;) {
        ...
        assert(next != NULL && "child returned NULL despite green peek showing a node");
