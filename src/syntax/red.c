...
_Static_assert(offsetof(struct SyntaxNode,  sll) == offsetof(struct SyntaxToken, sll),
               "SLL offset must match across SyntaxNode/Token");
_Static_assert(offsetof(struct SyntaxNode,  is_token) == offsetof(struct SyntaxToken, is_token),
               "is_token offset must match across SyntaxNode/Token");
_Static_assert(offsetof(struct SyntaxNode,  mutable_) == offsetof(struct SyntaxToken, mutable_),
               "mutable_ offset must match across SyntaxNode/Token");
_Static_assert(offsetof(struct SyntaxNode,  refcount) == offsetof(struct SyntaxToken, refcount),
               "refcount offset must match across SyntaxNode/Token");
_Static_assert(offsetof(struct SyntaxNode,  parent) == offsetof(struct SyntaxToken, parent),
               "parent offset must match across SyntaxNode/Token");
_Static_assert(offsetof(struct SyntaxNode,  index) == offsetof(struct SyntaxToken, index),
...
static SyntaxNode *node_child(SyntaxNode *parent, GreenNode *green,
    ...
    if (parent && parent->mutable_) {
        ...
        if (r.kind == SLL_ADD_ALREADY_IN_SLL) {
            ...
            assert(existing->is_token == 0 && "SLL slot occupied by token, expected node");
...
static SyntaxToken *token_child(SyntaxNode *parent, GreenToken *green,
    ...
    if (parent && parent->mutable_) {
        ...
        if (r.kind == SLL_ADD_ALREADY_IN_SLL) {
            ...
            assert(existing_n->is_token == 1 && "SLL slot occupied by node, expected token");
        ...
        if (!t) abort();
        init_common((SyntaxNode *)t, parent, index, offset, /*is_token=*/1, /*mutable_=*/1);
        t->green = green;
        syntax_node_retain(parent);
        sll_add(r, &t->sll);
        return t;
    ...
    SyntaxToken *t = (SyntaxToken *)malloc(sizeof(SyntaxToken));
    if (!t) abort();
    init_common((SyntaxNode *)t, parent, index, offset, /*is_token=*/1, /*mutable_=*/0);
    t->green = green;
    syntax_node_retain(parent);
    return t;
}
...
SyntaxKind syntax_node_kind(const SyntaxNode *n) { return green_node_kind(n->green); }
...
uint32_t syntax_node_num_children(const SyntaxNode *n) { return green_node_num_children(n->green); }

SyntaxKind syntax_token_kind(const SyntaxToken *t) { return green_token_kind(t->green); }
...
const char *syntax_token_text(const SyntaxToken *t) { return green_token_text(t->green); }
...
SyntaxNode *syntax_node_child(SyntaxNode *n, uint32_t i) {
    if (i >= green_node_num_children(n->green)) return NULL;
    GreenElement child = green_node_child(n->green, i);
    if (child.kind != GREEN_ELEM_NODE) return NULL;
...
SyntaxNode *syntax_node_prev_sibling(SyntaxNode *n) {
    if (!n->parent || n->index == 0) return NULL;
    SyntaxNode *parent = n->parent;
...
static SyntaxElement wrap_child(SyntaxNode *parent, uint32_t i) {
    ...
    if (!g.node && !g.token) return SYNTAX_ELEMENT_NONE;
    uint32_t off = node_abs_offset(parent) + g.rel_offset;
    if (g.kind == GREEN_ELEM_NODE) {
...
SyntaxElement syntax_node_prev_sibling_or_token(SyntaxNode *n) {
    if (!n->parent || n->index == 0) return SYNTAX_ELEMENT_NONE;
    return wrap_child(n->parent, n->index - 1);
}
...
SyntaxElement syntax_token_prev_sibling_or_token(SyntaxToken *t) {
    if (t->index == 0) return SYNTAX_ELEMENT_NONE;
    return wrap_child(t->parent, t->index - 1);
}
...
static SyntaxToken *element_leftmost_token(SyntaxElement e) {
    if (e.kind == SYNTAX_ELEM_TOKEN) return e.token;  // already a token
...
static SyntaxToken *element_rightmost_token(SyntaxElement e) {
    if (e.kind == SYNTAX_ELEM_TOKEN) return e.token;
...
TextRange syntax_node_child_range(const SyntaxNode *n, uint32_t i) {
    if (i >= green_node_num_children(n->green)) return TEXT_RANGE_NONE;
    GreenElement child = green_node_child(n->green, i);
...
// are RETURNS_OWNED.
static TokenAtOffset token_at_offset_recurse_elem(SyntaxElement e, uint32_t offset) {
...
static TokenAtOffset token_at_offset_recurse(SyntaxNode *node, uint32_t offset) {
    ...
    if (left_r.kind == TOKEN_AT_OFFSET_SINGLE && right_r.kind == TOKEN_AT_OFFSET_SINGLE) {
    ...
    if (left_r.kind  == TOKEN_AT_OFFSET_SINGLE) syntax_token_release(left_r.single);
    if (right_r.kind == TOKEN_AT_OFFSET_SINGLE) syntax_token_release(right_r.single);
    return (TokenAtOffset){.kind = TOKEN_AT_OFFSET_NONE};
}

TokenAtOffset syntax_token_at_offset(SyntaxNode *root, uint32_t offset) {
    if (!root) return (TokenAtOffset){.kind = TOKEN_AT_OFFSET_NONE};
...
SyntaxNode *syntax_node_clone_for_update(SyntaxNode *node) {
    ...
    SyntaxNode *clone = node_child(cloned_parent, node->green, node->index, node->offset);
...
SyntaxElement syntax_node_first_child_or_token_by_kind(SyntaxNode *n, SyntaxKind k) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        GreenElement g = green_node_child(n->green, i);
        SyntaxKind gk = (g.kind == GREEN_ELEM_NODE)
                           ? green_node_kind(g.node)
                           : green_token_kind(g.token);
        if (gk == k) return wrap_child(n, i);
    }
    return SYNTAX_ELEMENT_NONE;
}
...
SyntaxElement syntax_node_next_sibling_or_token_by_kind(SyntaxNode *n, SyntaxKind k) {
    ...
    for (uint32_t i = n->index + 1; i < count; i++) {
        ...
        SyntaxKind gk = (g.kind == GREEN_ELEM_NODE)
                           ? green_node_kind(g.node)
                           : green_token_kind(g.token);
        if (gk == k) return wrap_child(parent, i);
    }
    return SYNTAX_ELEMENT_NONE;
}
...
SyntaxElement syntax_node_covering_element(SyntaxNode *n, TextRange range) {
    ...
    for (;;) {
        if (cur.kind == SYNTAX_ELEM_TOKEN) return cur;
        SyntaxElement child = syntax_node_child_or_token_at_range(cur.node, range);
        if (syntax_element_is_none(child)) return cur;  // current is smallest
        // Descend.
...
static SyntaxNode *elem_data(SyntaxElement e) {
    if (e.kind == SYNTAX_ELEM_NODE) return e.node;
...
static void detach_common(SyntaxNode *node) {
    ...
    if (node->is_token) green_token_retain(((SyntaxToken *)node)->green);
    else green_node_retain(node->green);
...
static void attach_child(SyntaxNode *parent, uint32_t index, SyntaxElement child) {
    ...
    if (child.kind == SYNTAX_ELEM_NODE) {
        gelem = (GreenElement){.kind = GREEN_ELEM_NODE,  .node  = child_data->green,
                               .rel_offset = 0};
    } else {
        gelem = (GreenElement){.kind = GREEN_ELEM_TOKEN, .token = ((SyntaxToken *)child_data)->green,
                               .rel_offset = 0};
    }
    GreenNode *new_parent_green =
    ...
    if (child.kind == SYNTAX_ELEM_NODE) green_node_release(child_data->green);
    else                                 green_token_release(((SyntaxToken *)child_data)->green);
...
GreenNode *syntax_node_replace_with(SyntaxNode *node, GreenNode *replacement) {
    ...
    while (cur->parent) {
        GreenElement elem = {.kind = GREEN_ELEM_NODE, .node = current, .rel_offset = 0};
        GreenNode *up = green_node_replace_child(cur->parent->green, cur->index, elem);
...
GreenNode *syntax_token_replace_with(SyntaxToken *tok, GreenToken *replacement) {
    ...
    GreenElement elem = {.kind = GREEN_ELEM_TOKEN, .token = replacement,
                         .rel_offset = 0};
    GreenNode *new_parent_green =
    ...
    while (cur->parent) {
        GreenElement e = {.kind = GREEN_ELEM_NODE, .node = current, .rel_offset = 0};
