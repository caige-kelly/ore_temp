...
bool syntax_text_for_each_chunk(const SyntaxText *st,
  ...
  for (;;) {
    ...
    uint32_t clip_hi =
        (t_end > window_end) ? (tr.length - (t_end - window_end)) : tr.length;
...
int syntax_text_byte_at(const SyntaxText *st, uint32_t offset) {
  if (offset < st->range.start)
    return -1;
  if (offset >= text_range_end(st->range))
    return -1;
...
uint32_t syntax_text_find_byte(const SyntaxText *st, char c) {
  FindByteCtx ctx = {
      .needle = c, .cursor = st->range.start, .found_at = UINT32_MAX};
...
typedef struct {
  uint32_t target;
  uint32_t cursor;  // current absolute offset
  uint8_t carry[4]; // bytes from previous chunk that didn't form a complete
                    // sequence
  uint32_t carry_len;
...
static bool char_at_visit(const char *text, uint32_t len, void *user) {
  ...
  while (ctx->carry_len > 0 && i < len) {
    ...
    if ((uint32_t)needed > ctx->carry_len)
      continue; // need more
    uint32_t consumed;
...
int32_t syntax_text_char_at(const SyntaxText *st, uint32_t offset) {
  ...
  if (offset >= text_range_end(st->range))
    return -1;
  CharAtCtx ctx = {.target = offset,
                   .cursor = st->range.start,
                   .carry_len = 0,
                   .result = -1,
                   .found = false};
...
uint32_t syntax_text_find_char(const SyntaxText *st, int32_t c) {
  ...
  FindCharCtx ctx = {.needle = enc,
                     .needle_len = (uint32_t)nlen,
                     .cursor = st->range.start,
                     .found_at = UINT32_MAX,
                     .carry_len = 0};
