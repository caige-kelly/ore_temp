...
typedef struct {
  SyntaxKind kind;
  uint32_t first_child; // index in builder.children where this frame's children
                        // begin
} BuilderFrame;
...
void green_builder_token(GreenBuilder *b, SyntaxKind kind, const char *text,
  ...
  b->children[b->children_count++] = (GreenElement){
      ...
      .rel_offset =
          0, // assigned by node_alloc when this becomes part of a node
      .token = t,
  };
}
