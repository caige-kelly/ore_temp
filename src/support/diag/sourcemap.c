...
static struct SourceFile *source_file_for_path(struct SourceMap *map,
                                               const char *path) {
  if (!map || !path || !map->files)
    return NULL;
...
struct SourceFile *sourcemap_add_file(struct SourceMap *map,
                                      int requested_file_id, const char *path,
                                      char *source) {
  if (!map || !path || !source)
    return NULL;
  ...
  file->file_id =
      requested_file_id >= 0 ? requested_file_id : map->next_file_id;
  if (file->file_id >= map->next_file_id)
    map->next_file_id = file->file_id + 1;
...
struct SourceFile *sourcemap_file_for_id(struct SourceMap *map, int file_id) {
  ...
  for (size_t i = 0; i < map->files->count; i++) {
    struct SourceFile **file_p = (struct SourceFile **)vec_get(map->files, i);
    if (file_p && *file_p && (*file_p)->file_id == file_id)
      return *file_p;
  }
  return NULL;
}
...
const char *sourcemap_get_line(struct SourceMap *map, int file_id, int line,
  ...
  if (line_len)
    *line_len = 0;
  struct SourceFile *file = sourcemap_file_for_id(map, file_id);
  if (!file || line <= 0 || !file->line_starts)
    return NULL;
  size_t idx = (size_t)(line - 1);
  if (idx >= file->line_starts->count)
    return NULL;
