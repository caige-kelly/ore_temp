...
static void grow_page_array(PagedVec *p, size_t min_page_cap) {
    size_t new_cap = p->page_cap < PAGED_INITIAL_PAGE_CAP
                         ? PAGED_INITIAL_PAGE_CAP
                         : p->page_cap * 2;
    while (new_cap < min_page_cap) new_cap *= 2;

    void **new_pages = realloc(p->pages, new_cap * sizeof(*new_pages));
    if (!new_pages) abort();  // OOM on long-lived storage is unrecoverable
    ...
    for (size_t i = p->page_cap; i < new_cap; i++) new_pages[i] = NULL;
