...
static Precedence get_infix_precedence(TokenKind kind) {
    ...
    case TK_PIPE: case TK_AMP: case TK_CARET:
        return PREC_BITWISE;
...
static AstNodeKind get_binary_op_kind(TokenKind kind) {
    ...
    case TK_STAR_STAR: return AST_EXPR_BIN_POW;
...
static inline TinySpan p_node_span(const Parser *p, AstNodeId id) {
    return *(TinySpan*)vec_get(&p->mod->span_map, id.idx);
}

static inline TinySpan span_from_to(const Parser *p, const Token *start, const Token *end) {
    return span_make_range((uint16_t)p->mod->file.idx, start->start, end->byte_end);
}
...
static AstNodeId parse_param(Parser *p, bool name_required) {
    ...
    if (name_required) {
        const Token *name_tok = p_consume(p, TK_IDENTIFIER, "Expected parameter name");
        if (!name_tok) return AST_NODE_ID_NONE;
        AstNodeData nd = {0};
        nd.string_id = name_tok->string_id;
        name_id = p_push_node(p, AST_EXPR_PATH, p->pos - 1, nd, p_span(p, name_tok, name_tok));
    ...
    AstExtraDataIdx extra = ast_push_extra(p->mod->ast, payload, 3);
    AstNodeData data = {0};
    data.extra_idx = extra;

    const Token *end_tok = p_prev(p);
    return p_push_node(p, AST_DECL_VAL, op_index, data, span_from_to(p, first_tok, end_tok));
}
...
static AstNodeId parse_fn_lambda(Parser *p) {
    ...
    if (!p_consume(p, TK_LPAREN, "Expected '(' after fn")) return AST_NODE_ID_NONE;
    ...
    if (!p_match(p, TK_VOID)) {  // `fn(void)` is a zero-param lambda
        while (!p_is_eof(p) && p_peek(p) != TK_RPAREN) {
            ...
            if (param.idx && param_count < 256) params[param_count++] = param.idx;
    ...
    if (nx != TK_RPAREN && nx != TK_COMMA && nx != TK_GT &&
        nx != TK_SEMI && nx != TK_RBRACE && nx != TK_PIPE && nx != TK_EOF) {
        body = parse_expr(p, PREC_NONE);
    }

    uint32_t payload[260];
    payload[0] = ret_type.idx;
    payload[1] = body.idx;
    payload[2] = effect_id.idx;
    payload[3] = param_count;
    for (uint32_t i = 0; i < param_count; i++) payload[4 + i] = params[i];

    AstExtraDataIdx extra = ast_push_extra(p->mod->ast, payload, 4 + param_count);
    AstNodeData data = {0};
    data.extra_idx = extra;

    const Token *end_tok = p_prev(p);
    return p_push_node(p, AST_EXPR_LAMBDA, op_index, data, span_from_to(p, start_tok, end_tok));
}
...
static AstNodeId parse_fn_type(Parser *p) {
    ...
    while (!p_is_eof(p) && p_peek(p) != TK_RPAREN) {
        ...
        if (t.idx && param_count < 256) params[param_count++] = t.idx;
        if (!p_match(p, TK_COMMA)) break;
    }
    ...
    uint32_t payload[258];
    payload[0] = ret_type.idx;
    payload[1] = param_count;
    for (uint32_t i = 0; i < param_count; i++) payload[2 + i] = params[i];

    AstExtraDataIdx extra = ast_push_extra(p->mod->ast, payload, 2 + param_count);
    AstNodeData data = {0};
    data.extra_idx = extra;

    return p_push_node(p, AST_TYPE_FN, op_index, data, span_from_to(p, start_tok, p_prev(p)));
}
...
static AstNodeId parse_if_expr(Parser *p) {
    ...
    return p_push_node(p, AST_STMT_IF, op_index, data, span_from_to(p, start_tok, p_prev(p)));
}
...
static AstNodeId parse_loop_expr(Parser *p) {
    ...
    AstExtraDataIdx extra = ast_push_extra(p->mod->ast, payload, 4);
    AstNodeData data = {0};
    data.extra_idx = extra;

    return p_push_node(p, AST_STMT_LOOP, op_index, data, span_from_to(p, start_tok, p_prev(p)));
}
...
// Each variant: AST_DECL_VAL with extras [name_id, explicit_value_id (or NONE)].
// =============================================================================
...
static AstNodeId parse_enum_expr(Parser *p) {
    ...
    uint32_t variants[256];
    uint32_t variant_count = 0;

    while (!p_is_eof(p) && p_peek(p) != TK_RBRACE) {
        size_t pos_before = p->pos;

        const Token *name_tok = p_consume(p, TK_IDENTIFIER, "Expected variant name");
        if (!name_tok) break;
        AstNodeData nd = {0};
        nd.string_id = name_tok->string_id;
        AstNodeId name_id = p_push_node(p, AST_EXPR_PATH, p->pos - 1, nd, p_span(p, name_tok, name_tok));
        ...
        AstExtraDataIdx vextra = ast_push_extra(p->mod->ast, payload, 2);
        AstNodeData vdata = {0};
        vdata.extra_idx = vextra;
        AstNodeId variant = p_push_node(p, AST_DECL_VAL, p->pos - 1, vdata,
                                         span_from_to(p, name_tok, p_prev(p)));
        if (variant_count < 256) variants[variant_count++] = variant.idx;

        p_match(p, TK_SEMI);
        if (p->pos == pos_before) p_advance(p);
    }

    p_consume(p, TK_RBRACE, "Expected '}' to close enum");

    uint32_t payload[257];
    payload[0] = variant_count;
    for (uint32_t i = 0; i < variant_count; i++) payload[1 + i] = variants[i];
    AstExtraDataIdx extra = ast_push_extra(p->mod->ast, payload, 1 + variant_count);
    AstNodeData data = {0};
    data.extra_idx = extra;

    return p_push_node(p, AST_DECL_ENUM, op_index, data, span_from_to(p, start_tok, p_prev(p)));
}
...
static AstNodeId parse_switch_expr(Parser *p) {
    ...
    while (!p_is_eof(p) && p_peek(p) != TK_RBRACE) {
        ...
        for (;;) {
            ...
            if (pat.idx && pat_count < 16) pats[pat_count++] = pat.idx;
            if (!p_match(p, TK_PIPE)) break;
        }
        ...
        uint32_t payload[18];
        payload[0] = pat_count;
        for (uint32_t i = 0; i < pat_count; i++) payload[1 + i] = pats[i];
        payload[1 + pat_count] = body.idx;
        AstExtraDataIdx aextra = ast_push_extra(p->mod->ast, payload, 2 + pat_count);
        AstNodeData adata = {0};
        adata.extra_idx = aextra;
        AstNodeId arm = p_push_node(p, AST_DECL_VAL, p->pos - 1, adata,
                                     span_from_to(p, arm_start, p_prev(p)));
        if (arm_count < 256) arms[arm_count++] = arm.idx;
    ...
    uint32_t payload[258];
    payload[0] = scrutinee.idx;
    payload[1] = arm_count;
    for (uint32_t i = 0; i < arm_count; i++) payload[2 + i] = arms[i];
    AstExtraDataIdx extra = ast_push_extra(p->mod->ast, payload, 2 + arm_count);
    AstNodeData data = {0};
    data.extra_idx = extra;

    return p_push_node(p, AST_STMT_SWITCH, op_index, data, span_from_to(p, start_tok, p_prev(p)));
}
...
static AstNodeId parse_bracket_expr(Parser *p) {
    ...
    if (p_match(p, TK_UNDERSCORE)) {
        ...
        AstExtraDataIdx extra = ast_push_extra(p->mod->ast, payload, 2);
        AstNodeData data = {0};
        data.extra_idx = extra;
    ...
    if (p_match(p, TK_LBRACE)) {
        ...
        while (!p_is_eof(p) && p_peek(p) != TK_RBRACE) {
            ...
            if (p_peek(p) == TK_DOT && p_peek_at(p, 1) == TK_IDENTIFIER) {
                ...
                field_name = p_push_node(p, AST_EXPR_PATH, p->pos - 1, nd, p_span(p, nm, nm));
            ...
            AstExtraDataIdx fextra = ast_push_extra(p->mod->ast, payload, 2);
            AstNodeData fdata = {0};
            fdata.extra_idx = fextra;
            AstNodeId field = p_push_node(p, AST_DECL_VAL, p->pos - 1, fdata,
        ...
        uint32_t payload[258];
        payload[0] = AST_NODE_ID_NONE.idx; // type_expr (none for anonymous)
        payload[1] = field_count;
        for (uint32_t i = 0; i < field_count; i++) payload[2 + i] = fields[i];
        AstExtraDataIdx extra = ast_push_extra(p->mod->ast, payload, 2 + field_count);
        AstNodeData data = {0};
        data.extra_idx = extra;
...
static AstNodeId parse_builtin_expr(Parser *p) {
    ...
    const Token *name_tok = p_consume(p, TK_IDENTIFIER, "Expected builtin name after '@'");
    if (!name_tok) return AST_NODE_ID_NONE;
    ...
    if (p_match(p, TK_LPAREN)) {
        while (!p_is_eof(p) && p_peek(p) != TK_RPAREN) {
            ...
            if (arg.idx && arg_count < 64) args[arg_count++] = arg.idx;
            if (!p_match(p, TK_COMMA)) break;
        }
    ...
    uint32_t payload[66];
    payload[0] = name_tok->string_id.idx;
    payload[1] = arg_count;
    for (uint32_t i = 0; i < arg_count; i++) payload[2 + i] = args[i];
    AstExtraDataIdx extra = ast_push_extra(p->mod->ast, payload, 2 + arg_count);
    AstNodeData data = {0};
    data.extra_idx = extra;
...
static AstNodeId parse_prefix_unary(Parser *p, AstNodeKind kind) {
    ...
    TinySpan full = span_make_range((uint16_t)p->mod->file.idx,
                                     start_tok->start, span_end(op_span));
...
static AstNodeId parse_prefix(Parser *p) {
    ...
    case TK_FLOAT_LIT: {
        ...
        AstNodeKind k = (kind == TK_INT_LIT) ? AST_EXPR_LIT_INT : AST_EXPR_LIT_FLOAT;
    ...
    case TK_BYTE_LIT: {
        ...
        AstNodeData d = {0};
        d.string_id = start_tok->string_id;
        AstNodeKind k = (kind == TK_STRING_LIT) ? AST_EXPR_LIT_STRING : AST_EXPR_LIT_BYTE;
        return p_push_node(p, k, op_index, d, p_span(p, start_tok, start_tok));
    }
    case TK_ASM_LIT: {
        p_advance(p);
        AstNodeData d = {0};
        d.string_id = start_tok->string_id;
        return p_push_node(p, AST_EXPR_ASM, op_index, d, p_span(p, start_tok, start_tok));
    }
    case TK_TRUE:
    case TK_FALSE: {
        p_advance(p);
        ...
        return p_push_node(p, AST_EXPR_LIT_BOOL, op_index, d, p_span(p, start_tok, start_tok));
    }
    case TK_NIL: {
        p_advance(p);
        AstNodeData d = {0};
        return p_push_node(p, AST_EXPR_LIT_NIL, op_index, d, p_span(p, start_tok, start_tok));
    }
    ...
    case TK_UNDERSCORE: {
        p_advance(p);
        AstNodeData d = {0};
        return p_push_node(p, AST_EXPR_WILDCARD, op_index, d, p_span(p, start_tok, start_tok));
    }
    ...
    case TK_LPAREN: {
        ...
        return p_push_node(p, AST_EXPR_GROUP, op_index, d, p_span(p, start_tok, end_tok));
    }
    ...
    case TK_MINUS:    return parse_prefix_unary(p, AST_EXPR_UNARY_NEG);
    case TK_BANG:     return parse_prefix_unary(p, AST_EXPR_UNARY_NOT);
    case TK_TILDE:    return parse_prefix_unary(p, AST_EXPR_UNARY_BIT_NOT);
    case TK_AMP:      return parse_prefix_unary(p, AST_EXPR_UNARY_REF);
    case TK_STAR:     return parse_prefix_unary(p, AST_EXPR_UNARY_DEREF);

    // ---- Prefix unary (type position) ------------------------------
    case TK_CARET:    return parse_prefix_unary(p, AST_EXPR_UNARY_PTR);
    case TK_QUESTION: return parse_prefix_unary(p, AST_EXPR_UNARY_OPTIONAL);
    case TK_CONST:    return parse_prefix_unary(p, AST_EXPR_UNARY_CONST);
    ...
    // ---- Control flow ----------------------------------------------
    case TK_IF:
    case TK_ELIF:      return parse_if_expr(p);
    case TK_LOOP:      return parse_loop_expr(p);
    case TK_RETURN:    return parse_return_expr(p);
    case TK_DEFER:     return parse_defer_expr(p);
    case TK_BREAK:     return parse_break_or_continue(p, AST_STMT_BREAK);
    case TK_CONTINUE:  return parse_break_or_continue(p, AST_STMT_CONTINUE);

    // ---- Array / bracket forms -------------------------------------
    case TK_LBRACKET:  return parse_bracket_expr(p);

    // ---- Dot forms -------------------------------------------------
    case TK_DOT:       return parse_dot_expr(p);

    // ---- Compiler builtins -----------------------------------------
    case TK_AT:        return parse_builtin_expr(p);
    ...
    case TK_MASK:
    case TK_WITH: {
        p_error(p, "effect / handler / mask / with not yet implemented in new parser");
        p_advance(p);
        return AST_NODE_ID_NONE;
    }

    // ---- distinct T ------------------------------------------------
    case TK_DISTINCT:  return parse_prefix_unary(p, AST_DECL_DISTINCT);
...
static AstNodeId parse_infix(Parser *p, AstNodeId left, TinySpan left_span) {
    ...
    if (kind == TK_LPAREN) {
        ...
        while (!p_is_eof(p) && p_peek(p) != TK_RPAREN) {
            AstNodeId arg = parse_expr(p, PREC_NONE);
            if (arg_count < 256) args[arg_count++] = arg.idx;
            if (!p_match(p, TK_COMMA)) break;
        }
