...
static bool is_assign_op (OreSyntaxKind k) { return ore_kind_is_assign_op_token(k); }
static bool is_prefix_op (OreSyntaxKind k) { return ore_kind_is_prefix_op_token(k); }
static bool is_postfix_op(OreSyntaxKind k) { return ore_kind_is_postfix_op_token(k); }
static bool is_literal_tok(OreSyntaxKind k) {
    return ore_kind_is_literal_token(k)
        || k == SK_TRUE_KW || k == SK_FALSE_KW || k == SK_NIL_KW;
}
...
SyntaxToken *BinExpr_op (const BinExpr *b)  { return ast_first_token_pred(b->syntax, is_bin_op); }
SyntaxKind   BinExpr_op_kind(const BinExpr *b) {
    SyntaxToken *t = BinExpr_op(b);
    if (!t) return SYNTAX_KIND_NONE;
...
SyntaxNode  *AssignExpr_lhs(const AssignExpr *a) { return nth_expr(a->syntax, 0); }
SyntaxNode  *AssignExpr_rhs(const AssignExpr *a) { return nth_expr(a->syntax, 1); }
SyntaxToken *AssignExpr_op (const AssignExpr *a) { return ast_first_token_pred(a->syntax, is_assign_op); }
...
SyntaxNode  *PrefixExpr_operand(const PrefixExpr *p) { return nth_expr(p->syntax, 0); }


// ---- PostfixExpr ----------------------------------------------------

SyntaxNode  *PostfixExpr_operand(const PostfixExpr *p) { return nth_expr(p->syntax, 0); }
...
SyntaxNode *CallExpr_callee(const CallExpr *c) { return nth_expr(c->syntax, 0); }
SyntaxNode *CallExpr_args  (const CallExpr *c) { return ast_first_child(c->syntax, SK_ARG_LIST); }


// ---- IndexExpr ------------------------------------------------------

SyntaxNode *IndexExpr_base (const IndexExpr *i) { return nth_expr(i->syntax, 0); }
SyntaxNode *IndexExpr_index(const IndexExpr *i) { return nth_expr(i->syntax, 1); }


// ---- SliceExpr ------------------------------------------------------

SyntaxNode *SliceExpr_base(const SliceExpr *s) { return nth_expr(s->syntax, 0); }
...
SyntaxNode  *FieldExpr_base (const FieldExpr *f) { return nth_expr(f->syntax, 0); }
SyntaxToken *FieldExpr_field(const FieldExpr *f) { return ast_first_token(f->syntax, SK_IDENT); }


// ---- RefExpr --------------------------------------------------------

SyntaxToken *RefExpr_name(const RefExpr *r) { return ast_first_token(r->syntax, SK_IDENT); }


// ---- ParenExpr ------------------------------------------------------

SyntaxNode *ParenExpr_inner(const ParenExpr *p) { return nth_expr(p->syntax, 0); }
...
SyntaxNode *IfExpr_then_branch(const IfExpr *i) { return nth_block_or_if(i->syntax, 0); }
SyntaxNode *IfExpr_else_branch(const IfExpr *i) { return nth_block_or_if(i->syntax, 1); }
