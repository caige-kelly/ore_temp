...
static struct TypeOfExprEntry *type_of_expr_entry_for(struct Sema *s,
  ...
  *e = (struct TypeOfExprEntry){0};
  ...
  hashmap_put_or_die(&s->type_of_expr_entries, key, e, "type_of_expr_entries");
  return e;
}
...
static const char *name_of(struct Sema *s, StrId name_id) {
...
struct Type *query_type_of_expr(struct Sema *s, struct Expr *expr) {
  ...
  switch (expr->kind) {
  case expr_Lit:
    result = type_of_lit(s, expr);
    break;
  case expr_Ident:
    result = type_of_ident(s, expr);
    break;
  case expr_Bin:
    result = type_of_bin(s, expr);
    break;
  case expr_Unary:
    result = type_of_unary(s, expr);
    break;
  case expr_Call:
    result = type_of_call(s, expr);
    break;
  case expr_Lambda:
    result = type_of_lambda(s, expr);
    break;
  case expr_Block:
    result = type_of_block(s, expr);
    break;
  case expr_If:
    result = type_of_if(s, expr);
    break;
  case expr_Index:
    result = type_of_index(s, expr);
    break;
  case expr_Slice:
    result = type_of_slice(s, expr);
    break;
  case expr_ArrayType:
  case expr_SliceType:
  ...
  if (expr && expected && expected->kind != TY_ERROR) {
    ...
    if (expr->kind == expr_Block && expr->block.stmts &&
      ...
      for (size_t i = 0; i < last; i++) {
        ...
        if (stmt)
          (void)query_type_of_expr(s, stmt);
      }
...
static struct Type *type_of_lit(struct Sema *s, struct Expr *e) {
  ...
  case lit_False:
    return s->bool_type;
  case lit_Nil:
    return s->nil_type ? s->nil_type : s->error_type;
  }
  return s->error_type;
}
...
static struct Type *bin_arith_result(struct Sema *s, struct Type *l,
  ...
  if (l_many || r_many) {
    if (e->bin.op == Plus) {
      if (l_many && type_is_int(r))
        return l;
      if (r_many && type_is_int(l))
        return r;
  ...
  if (type_is_comptime(l) && !type_is_comptime(r)) {
    ...
    if (!coerce(s, l, r, lv, e->bin.Left->span))
      return s->error_type;
    return r;
  }
  if (!type_is_comptime(l) && type_is_comptime(r)) {
    struct ConstValue rv = query_const_eval(s, e->bin.Right);
    if (!coerce(s, r, l, rv, e->bin.Right->span))
      return s->error_type;
    return l;
  }
  ...
  if (l != r) {
    char lb[64], rb[64];
    diag_error(
        &s->diags, e->span, "binary operator '%s': type mismatch — %s vs %s",
        token_kind_to_str(e->bin.op), type_to_string(s, l, lb, sizeof(lb)),
        type_to_string(s, r, rb, sizeof(rb)));
    return s->error_type;
  }
  return l;
...
static struct Type *bin_cmp_result(struct Sema *s, struct Type *l,
  ...
  if (type_is_numeric(l) && type_is_numeric(r)) {
    ...
    ok = (l == r) && (e->bin.op == EqualEqual || e->bin.op == BangEqual);
  }

  if (!ok) {
    char lb[64], rb[64];
    diag_emit(
        s, e->span, "comparison '%s' between incompatible types %s and %s",
        token_kind_to_str(e->bin.op), type_to_string(s, l, lb, sizeof(lb)),
        type_to_string(s, r, rb, sizeof(rb)));
    return s->error_type;
  }
...
static struct Type *bin_logical_result(struct Sema *s, struct Type *l,
  ...
  if (l->kind != TY_BOOL || r->kind != TY_BOOL) {
    char lb[64], rb[64];
    diag_emit(s, e->span, "logical '%s' requires bool operands; got %s and %s",
              token_kind_to_str(e->bin.op),
              type_to_string(s, l, lb, sizeof(lb)),
              type_to_string(s, r, rb, sizeof(rb)));
    return s->error_type;
...
static struct Type *type_of_unary(struct Sema *s, struct Expr *e) {
  ...
  case unary_Neg:
    if (!type_is_numeric(t)) {
      char b[64];
      diag_emit(s, e->span, "unary '-' requires a numeric operand; got %s",
  ...
  case unary_BitNot:
    if (!type_is_int(t)) {
      char b[64];
      diag_emit(s, e->span, "unary '~' requires an integer operand; got %s",
                type_to_string(s, t, b, sizeof(b)));
      return s->error_type;
    }
    return t;
  case unary_Not:
    if (t->kind != TY_BOOL) {
      char b[64];
      diag_emit(s, e->span, "unary '!' requires a bool operand; got %s",
                type_to_string(s, t, b, sizeof(b)));
      return s->error_type;
    }
    return s->bool_type;
  ...
  case unary_Deref: {
    ...
    if (t->kind != TY_PTR) {
      char b[64];
      diag_emit(s, e->span, "deref '^' requires a single pointer ^T; got %s",
                type_to_string(s, t, b, sizeof(b)));
      return s->error_type;
    }
    return t->ptr.elem;
  ...
  case unary_DeNil: {
    ...
    if (t->kind != TY_OPTIONAL) {
      char b[64];
      diag_emit(s, e->span, "postfix '?' requires an optional ?T; got %s",
                type_to_string(s, t, b, sizeof(b)));
      return s->error_type;
    }
...
static struct Type *type_of_call(struct Sema *s, struct Expr *e) {
  ...
  if (arg_count != callee_t->fn.param_count) {
    diag_emit(s, e->span, "wrong number of arguments: expected %zu, got %zu",
...
static struct Type *type_of_index(struct Sema *s, struct Expr *e) {
  ...
  if (obj->kind == TY_ERROR || idx->kind == TY_ERROR)
    return s->error_type;
  if (!type_is_int(idx)) {
    char b[64];
    diag_emit(s, e->index.index->span, "array index must be an integer; got %s",
  ...
  switch (obj->kind) {
  case TY_ARRAY:
    return obj->array.elem;
  case TY_SLICE:
    return obj->slice.elem;
  case TY_MANY_PTR:
...
static struct Type *type_of_slice(struct Sema *s, struct Expr *e) {
  ...
  if (e->slice.end) {
    ...
    if (et->kind != TY_ERROR && !type_is_int(et)) {
      char b[64];
      diag_emit(s, e->slice.end->span, "slice end must be an integer; got %s",
...
static size_t struct_find_field(struct StructSignature *sig, StrId name_id) {
  for (size_t i = 0; i < sig->field_count; i++) {
    if (sig->fields[i].name_id.v == name_id.v)
      return i;
  }
  return (size_t)-1;
}

static size_t enum_find_variant(struct EnumSignature *sig, StrId name_id) {
  for (size_t i = 0; i < sig->variant_count; i++) {
    if (sig->variants[i].name_id.v == name_id.v)
      return i;
  }
  return (size_t)-1;
}
...
static bool name_is(struct Sema *s, StrId id, const char *lit) {
...
static struct Type *type_of_field(struct Sema *s, struct Expr *e) {
  ...
  if (recv->kind == TY_ERROR)
    return s->error_type;

  StrId fname = e->field.field.string_id;
  ...
  if (recv->kind == TY_STRUCT) {
    ...
    if (!visibility_allows_external(sig->fields[idx].vis)) {
      ...
      if (module_id_is_valid(struct_module) &&
        ...
        char rb[64];
        diag_emit(s, e->field.field.span, "field '%s' on %s is private",
                  name_of(s, fname), type_to_string(s, recv, rb, sizeof(rb)));
      }
    }
  ...
  char b[64];
  diag_emit(s, e->field.object->span, "field access on non-struct/enum type %s",
...
static struct Type *type_of_product(struct Sema *s, struct Expr *e,
  ...
  if (e->product.type_expr) {
    ...
    if (!target || target->kind == TY_ERROR)
      return s->error_type;
  ...
  if (target->kind != TY_STRUCT) {
    char b[64];
    diag_emit(s, e->span, "struct literal expects a struct type; got %s",
...
static struct Type *type_of_enum_ref(struct Sema *s, struct Expr *e,
  ...
  if (!expected || expected->kind != TY_ENUM) {
    diag_emit(s, e->span, "enum literal '.%s' requires an enum-typed context",
  ...
  if (def_id_is_valid(fn_def)) {
    ...
    if (sig && sig->ret_type) {
      if (e->return_expr.value)
        ...
        char rb[64];
        diag_emit(s, e->span, "return without a value in fn returning %s",
...
static struct Type *type_of_loop(struct Sema *s, struct Expr *e) {
  if (e->loop_expr.init)
    (void)query_type_of_expr(s, e->loop_expr.init);
  ...
  if (e->loop_expr.step)
    (void)query_type_of_expr(s, e->loop_expr.step);
  if (e->loop_expr.body)
    (void)query_type_of_expr(s, e->loop_expr.body);
  return s->void_type;
}
...
static struct Type *type_of_array_lit(struct Sema *s, struct Expr *e) {
  ...
  if (e->array_lit.elem_type) {
    ...
    diag_emit(s, e->span, "array literal requires an explicit element type");
    return s->error_type;
  }
  ...
  Vec *elems =
      (init && init->kind == expr_Product) ? init->product.Fields : NULL;
...
static struct Expr *builtin_arg(struct Expr *e, size_t i) {
  if (!e->builtin.args || i >= e->builtin.args->count)
    return NULL;
...
static struct Type *type_of_builtin(struct Sema *s, struct Expr *e) {
  StrId nm = e->builtin.name_id;

  if (nm.v == s->name_sizeOf.v || nm.v == s->name_alignOf.v) {
    if (!require_arg_count(s, e, 1))
      return s->error_type;
  ...
  if (nm.v == s->name_intCast.v) {
    if (!require_arg_count(s, e, 2))
      return s->error_type;
...
static bool pattern_matches_enum_variant(struct Sema *s, struct Expr *pat,
  ...
  if (!pat || !sig)
    return false;
  if (pat->kind != expr_EnumRef)
    return false;
  StrId name = pat->enum_ref_expr.name.string_id;
  for (size_t i = 0; i < sig->variant_count; i++) {
    if (sig->variants[i].name_id.v == name.v) {
      if (out_idx)
        *out_idx = i;
      return true;
    }
