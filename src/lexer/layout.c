...
static struct Token layout_token(enum TokenKind kind, struct Span* span, StringPool* pool) {
    struct Token t = {
        .kind = kind,
...
static bool is_start_continuation(enum TokenKind kind, enum TokenKind next) {
    ...
    switch (kind) {
        ...
        case Plus: case ForwardSlash: case Percent:
...
static bool is_end_continuation(enum TokenKind kind) {
    switch (kind) {
        ...
        // Connectors
        case Dot: case RightArrow: case Colon:
        // Binary operators
...
static void maybe_insert_semicolon(struct LayoutNormalizer* ln, struct Token* token, StringPool* pool) {
    // No previous token on this line — nothing to separate
    if (ln->line_last_sig == Eof) return;
    
    // Expression continues — no semicolon
    if (is_end_continuation(ln->line_last_sig)) return;
    // peek the token after current for the dot check
    enum TokenKind next_kind = peek_next_kind(ln);
    if (is_start_continuation(token->kind, next_kind)) return;
    
    // Don't double-emit semicolons
    struct Token* last = (struct Token*)vec_get(ln->output, ln->output->count - 1);
    if (last && last->kind == Semicolon) return;
    
    // Don't emit after layout { or }
    if (last && last->kind == LBrace && last->origin == Layout) return;
...
static void handle_line_start(struct LayoutNormalizer* ln, struct Token* token, StringPool* pool) {
    ...
    struct LayoutFrame* top = (struct LayoutFrame*)vec_get(ln->frames, ln->frames->count - 1);
...
static void handle_open_brace(struct LayoutNormalizer* ln, struct Token* token) {
...
static void handle_close_brace(struct LayoutNormalizer* ln, struct Token* token, StringPool* pool) {
