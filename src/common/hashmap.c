...
static HashMapEntry* entries_alloc(HashMap* map, size_t capacity) {
    if (capacity > SIZE_MAX / sizeof(HashMapEntry)) return NULL;

    size_t bytes = capacity * sizeof(HashMapEntry);
    if (map->arena) return arena_alloc(map->arena, bytes);
...
static size_t next_capacity(size_t current, size_t min_capacity) {
    ...
    if (current) {
        if (current > SIZE_MAX / 2) return 0;
        capacity = current * 2;
    }
...
static void insert_existing(HashMapEntry* entries, size_t capacity, uint64_t key, void* value) {
...
static bool hashmap_grow(HashMap* map, size_t min_capacity) {
    ...
    for (size_t i = 0; i < map->capacity; i++) {
        ...
        if (entry->occupied) insert_existing(entries, capacity, entry->key, entry->value);
    }
...
static bool hashmap_needs_grow(HashMap* map) {
    ...
    if (map->count == SIZE_MAX) return true;
