...
static bool ip_key_eql(IpKey a, IpKey b) {
    ...
    case IPK_FN_TYPE:
        if (a.fn_type.ret.v != b.fn_type.ret.v) return false;
        if (a.fn_type.modifiers != b.fn_type.modifiers) return false;
        if (a.fn_type.n_params != b.fn_type.n_params) return false;
        for (size_t i = 0; i < a.fn_type.n_params; i++)
            if (a.fn_type.params[i].v != b.fn_type.params[i].v) return false;
        return true;
    case IPK_STRUCT_TYPE:
    ...
    case IPK_EFFECT_ROW:
        if (a.effect_row.n_effects != b.effect_row.n_effects) return false;
...
IpTag ip_tag(InternPool *pool, IpIndex idx) {
    if (idx.v >= pool->items_count) return IP_TAG_REMOVED;
...
bool ip_is_value(InternPool *pool, IpIndex idx) {
    IpTag t = ip_tag(pool, idx);
    return t == IP_TAG_RESERVED_VALUE ||
           t == IP_TAG_INT_VALUE ||
...
static uint32_t encode_payload(InternPool *pool, IpKey key, IpTag tag) {
    ...
    case IP_TAG_FN_TYPE: {
        ...
        if (n > 0) memcpy(p->params, key.fn_type.params, n * sizeof(IpIndex));
        return off;
    }
    case IP_TAG_STRUCT_TYPE: {
    ...
    case IP_TAG_ENUM_TYPE: {
        ...
        if (n > 0) {
            int64_t *values = arena_alloc_raw(&pool->extra_arena,
                                              n * sizeof(int64_t));
    ...
    case IP_TAG_EFFECT_ROW: {
        ...
        if (n > 0) memcpy(p->effects, key.effect_row.effects, n * sizeof(DefId));
        return off;
    }
    case IP_TAG_INT_VALUE: {
        uint32_t off = (uint32_t)arena_total_used(&pool->extra_arena);
...
static IpTag tag_for_key(IpKey key) {
    switch (key.kind) {
    case IPK_PRIMITIVE_TYPE: return IP_TAG_PRIMITIVE_TYPE;
    case IPK_RESERVED_VALUE: return IP_TAG_RESERVED_VALUE;
    case IPK_PTR_TYPE:
    ...
    case IPK_SLICE_TYPE:
        return key.slice_type.is_const ? IP_TAG_SLICE_CONST_TYPE
                                       : IP_TAG_SLICE_TYPE;
    case IPK_OPTIONAL_TYPE:  return IP_TAG_OPTIONAL_TYPE;
    case IPK_ARRAY_TYPE:     return IP_TAG_ARRAY_TYPE;
    case IPK_FN_TYPE:        return IP_TAG_FN_TYPE;
    case IPK_STRUCT_TYPE:    return IP_TAG_STRUCT_TYPE;
    case IPK_ENUM_TYPE:      return IP_TAG_ENUM_TYPE;
    case IPK_EFFECT_ROW:     return IP_TAG_EFFECT_ROW;
    case IPK_INT_VALUE:      return IP_TAG_INT_VALUE;
    case IPK_FLOAT_VALUE:    return IP_TAG_FLOAT_VALUE;
    }
...
static uint32_t encode_items_data(InternPool *pool, IpKey key, IpTag tag) {
    switch (tag) {
    case IP_TAG_PTR_TYPE:
    case IP_TAG_PTR_CONST_TYPE:           return key.ptr_type.elem.v;
    case IP_TAG_MANY_PTR_TYPE:
    case IP_TAG_MANY_PTR_CONST_TYPE:      return key.many_ptr_type.elem.v;
    case IP_TAG_SLICE_TYPE:
    case IP_TAG_SLICE_CONST_TYPE:         return key.slice_type.elem.v;
    case IP_TAG_OPTIONAL_TYPE:            return key.optional_type.elem.v;
    default:                              return encode_payload(pool, key, tag);
    }
}
...
IpIndex ip_get(InternPool *pool, IpKey key) {
    ...
    switch (key.kind) {
    case IPK_PRIMITIVE_TYPE: return (IpIndex){(uint32_t)key.primitive_type};
    case IPK_RESERVED_VALUE: return (IpIndex){(uint32_t)key.reserved_value};
    default: break;
    }
...
void ip_init_with(InternPool *pool, size_t initial_buckets,
    ...
    assert(initial_buckets >= 16 && (initial_buckets & (initial_buckets - 1)) == 0
...
WipContainerType ip_wip_struct(InternPool *pool, uint32_t zir_node_id,
    ...
    uint64_t h  = hash_key(k);
    uint32_t hh = (uint32_t)(h >> 32);
    if ((pool->bucket_used + 1) * 4 > pool->bucket_count * 3) buckets_grow(pool);
    size_t mask = pool->bucket_count - 1;
    size_t b    = (size_t)(h & mask);
    while (pool->buckets[b] != 0) b = (b + 1) & mask;
...
void ip_wip_struct_finish(InternPool *pool, WipContainerType wip,
    ...
    IpStructPayload *orig = arena_get_ptr(&pool->extra_arena,
                                          pool->items_data[wip.index.v]);
...
WipContainerType ip_wip_fn_type(InternPool *pool, uint32_t modifiers,
    ...
    for (size_t i = 0; i < n_params; i++) p->params[i] = IP_NONE;
...
void ip_wip_fn_finish(InternPool *pool, WipContainerType wip,
    ...
    p->modifiers = modifiers;
    p->n_params  = (uint32_t)n_params;
    if (n_params > 0) memcpy(p->params, params, n_params * sizeof(IpIndex));
...
void ip_remove(InternPool *pool, IpIndex idx) {
    if (idx.v >= pool->items_count) return;
...
void ip_dump_stats(InternPool *pool, FILE *out) {
    ...
    for (size_t i = 0; i < pool->items_count; i++) {
        if (pool->items_tag[i] == IP_TAG_REMOVED) { removed++; continue; }
...
static void format_recursive(FmtBuf *fb, InternPool *pool, IpIndex idx,
                             int depth) {
    if (depth > IP_FORMAT_MAX_DEPTH) { fb_puts(fb, "..."); return; }

    if (idx.v >= pool->items_count) { fb_puts(fb, "<oob>"); return; }
    if (pool->items_tag[idx.v] == IP_TAG_REMOVED) {
        fb_puts(fb, "<removed>"); return;
    }
    ...
    case IPK_RESERVED_VALUE:
        ...
        case IP_INDEX_BOOL_TRUE:    fb_puts(fb, "true");      break;
        case IP_INDEX_BOOL_FALSE:   fb_puts(fb, "false");     break;
        case IP_INDEX_VOID_VALUE:   fb_puts(fb, "void");      break;
        case IP_INDEX_UNDEF_VALUE:  fb_puts(fb, "undef");     break;
        case IP_INDEX_ZERO_USIZE:   fb_puts(fb, "0_usize");   break;
        case IP_INDEX_ONE_USIZE:    fb_puts(fb, "1_usize");   break;
        default:                    fb_puts(fb, "<bad-reserved>");
        }
        break;
    case IPK_PTR_TYPE:
