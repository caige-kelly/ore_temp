...
static StrId decl_name_of(struct db *s, SyntaxNode *decl) {
    ...
    if (k == SK_CONST_DECL) {
        ConstDef cd;
        if (ConstDef_cast(decl, &cd)) nt = ConstDef_name(&cd);
    } else if (k == SK_VAR_DECL) {
        VarDef vd;
        if (VarDef_cast(decl, &vd)) nt = VarDef_name(&vd);
    }
    if (!nt) return (StrId){0};
...
DefId db_node_enclosing_def(db_query_ctx *ctx, FileId fid, SyntaxNode *node) {
    ...
    if (!parent) return DEF_ID_NONE;                 // node IS the root
    ...
    for (;;) {
        ...
        if (cur_owned) syntax_node_release(cur);
        cur = parent;       cur_owned = true;   // cur takes parent's ref
        parent = grand;                          // parent takes grand's ref
    }

    StrId name = decl_name_of(s, cur);
    if (cur_owned) syntax_node_release(cur);
    if (name.idx == 0) return DEF_ID_NONE;

    NamespaceId nsid = db_get_file_namespace(s, fid);
    if (!namespace_id_valid(nsid)) return DEF_ID_NONE;
...
IpIndex db_query_node_type(db_query_ctx *ctx, FileId fid, SyntaxNode *node) {
    ...
    if (d.idx != 0) {
        ...
        if (k == KIND_FUNCTION) {
            ...
            NodeTypesRange r = {0};
            switch (k) {
            case KIND_STRUCT:   r = struct_field_node_types_read(s, d);   break;
            case KIND_VARIABLE: r = variable_value_node_types_read(s, d); break;
            case KIND_CONSTANT: r = constant_value_node_types_read(s, d); break;
            default: break;
            }
