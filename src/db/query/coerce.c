...
// Stored raw: a real type binding is IpIndex.v ≥ 1 (IP_NONE is index 0), so it
// never collides with NULL/absent. The only value that would store as NULL is
// IP_NONE itself, which is semantically "no binding" anyway — so a NULL read is
// correctly "unbound". (Pre-IP_NONE=0 this needed a +1 bias because bool was
// index 0.)
