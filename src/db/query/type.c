...
IpIndex node_types_range_lookup(struct db *s, NodeTypesRange range, SyntaxNode *node) {
    (void)s;
...
static SyntaxNode *bind_value(SyntaxNode *wrapper) {
    switch ((OreSyntaxKind)syntax_node_kind(wrapper)) {
        case SK_CONST_DECL: { ConstDef c; if (ConstDef_cast(wrapper, &c)) return ConstDef_value(&c); break; }
        case SK_VAR_DECL:   { VarDef v;   if (VarDef_cast(wrapper, &v))   return VarDef_value(&v);   break; }
        default: break;
    }
    return NULL;
}
static SyntaxNode *bind_type_annot(SyntaxNode *wrapper) {
    switch ((OreSyntaxKind)syntax_node_kind(wrapper)) {
        case SK_CONST_DECL: { ConstDef c; if (ConstDef_cast(wrapper, &c)) return ConstDef_type(&c); break; }
        case SK_VAR_DECL:   { VarDef v;   if (VarDef_cast(wrapper, &v))   return VarDef_type(&v);   break; }
        default: break;
    }
    return NULL;
}
...
static IpIndex build_fn_type(const SemaCtx *ctx, SyntaxNode *ret_node,
    ...
    if (param_list) {
        ...
        for (uint32_t i = 0; i < pc; i++) {
            ...
            Param p;
            if (!Param_cast(el.node, &p)) { syntax_node_release(el.node); continue; }
            SyntaxNode *ptype = Param_type(&p);
            IpIndex pti = ptype ? resolve_type_expr(ctx, ptype) : IP_NONE;
            if (ptype) syntax_node_release(ptype);
            if (pti.v == IP_NONE.v) { syntax_node_release(el.node); return IP_NONE; }
            params[out++] = pti;
    ...
    if (!ret_node) {
        ...
        if (ret.v == IP_NONE.v) return IP_NONE;
    }

    IpKey key = {.kind = IPK_FN_TYPE,
                 .fn_type = {.ret = ret, .modifiers = 0,
                             .params = params, .n_params = n_params},
...
IpIndex resolve_type_expr(const SemaCtx *ctx, SyntaxNode *node) {
    ...
    case SK_CONST_TYPE: {
        ...
        if (ConstType_cast(node, &ct)) {
            ...
            if (inner) { result = resolve_type_expr(ctx, inner); syntax_node_release(inner); }
        }
        break;
    }
    ...
    case SK_MANY_PTR_TYPE: {
        ...
        if (k == SK_PTR_TYPE) {
            ...
            ManyPtrType mt; ManyPtrType_cast(node, &mt); child = ManyPtrType_element(&mt);
        }
        bool is_const = false;
        ...
        if (child) syntax_node_release(child);
        if (elem.v != IP_NONE.v) {
            IpKey key = {0};
            if (k == SK_PTR_TYPE) {
                key.kind = IPK_PTR_TYPE; key.ptr_type.elem = elem; key.ptr_type.is_const = is_const;
            } else if (k == SK_SLICE_TYPE) {
                key.kind = IPK_SLICE_TYPE; key.slice_type.elem = elem; key.slice_type.is_const = is_const;
            } else {
                key.kind = IPK_MANY_PTR_TYPE; key.many_ptr_type.elem = elem; key.many_ptr_type.is_const = is_const;
            }
            result = ip_get(&s->intern, key);
        }
        break;
    ...
    case SK_ARRAY_TYPE: {
        ...
        if (!size_node) {
            db_emit(s, DIAG_ERROR, span_of(ctx, node), "array type missing size expression");
        ...
        if (elem.v == IP_NONE.v)
            break;
        IpKey key = {.kind = IPK_ARRAY_TYPE, .array_type = {.elem = elem, .size = size}};
        result = ip_get(&s->intern, key);
        break;
    }
...
static IpIndex build_struct_type(const SemaCtx *base, SyntaxNode *agg, DefId def,
                                 Fingerprint *fp_out) {
    struct db *s = base->s;
    DefKind dk = db_def_kind(s, def);  // KIND_STRUCT or KIND_UNION

    IpIndex idx = ip_get(&s->intern,
                         (IpKey){.kind = IPK_STRUCT_TYPE,
    ...
    *db_def_type_cell(s, def) = idx;   // self-reference anchor
    ...
    if (ak == SK_STRUCT_DECL) {
        StructDef sd; if (StructDef_cast(agg, &sd)) field_list = StructDef_fields(&sd);
    } else if (ak == SK_UNION_DECL) {
        UnionDef ud; if (UnionDef_cast(agg, &ud)) field_list = UnionDef_variants(&ud);
    }
    ...
    if (field_list) {
        ...
        for (uint32_t i = 0; i < list_count; i++) {
            ...
            if (syntax_node_kind(el.node) != SK_FIELD) { syntax_node_release(el.node); continue; }
            Field fld;
            if (!Field_cast(el.node, &fld)) { syntax_node_release(el.node); continue; }
...
static IpIndex build_enum_type(const SemaCtx *base, SyntaxNode *enum_node,
    ...
    EnumVariantEntry *scratch =
        n_variants ? arena_alloc(&s->request_arena, n_variants * sizeof(EnumVariantEntry))
                   : NULL;
    ...
    if (variants_list) {
        ...
        for (uint32_t i = 0; i < list_count; i++) {
            ...
            if (syntax_node_kind(el.node) != SK_VARIANT) { syntax_node_release(el.node); continue; }
            Variant v;
            if (!Variant_cast(el.node, &v)) { syntax_node_release(el.node); continue; }
            ...
            if (v_val) syntax_node_release(v_val);
            scratch[vout++] = (EnumVariantEntry){.name = vname, .value = val};
            content = db_fp_combine(content,
                                    db_fp_combine(db_fp_u64((uint64_t)vname.idx),
        ...
        if (groot) {
            ...
            if (wrapper) {
                ...
                if (value && LambdaExpr_cast(value, &lam)) {
                    ...
                    SemaCtx sctx = {.s = s, .file_green_root = groot, .nsid = nsid,
                                    .enclosing_fn = def, .file_local = e.file, .types = &sb};
