...
static void record_dep_on_parent(db_query_ctx *ctx, QueryKind child_kind,
  ...
  if (!deduped) {
    ...
    if (!existing)
      hashmap_put_or_die(parent->dep_index, ikey, (void *)(uintptr_t)row,
                         "engine: dep dedup");
  }
...
bool db_engine_route_slot(db_query_ctx *ctx, QueryKind kind, uint64_t key,
  ...
  case QUERY_SOURCE_TEXT: {
    ...
    if (local >= s->sources.slots_text_hot.count)
      return false;
  ...
  case QUERY_LINE_INDEX: {
    FileId f = {.idx = (uint32_t)key};
    uint32_t local = file_id_local(f);
    if (local >= s->files.slots_line_index_hot.count)
      return false;
  ...
  case QUERY_FILE_SET: {
    ...
    if (nsid >= s->namespaces.slots_file_set_hot.count)
      return false;
  ...
  case QUERY_NAMESPACE_SCOPES: {
    uint32_t nsid = (uint32_t)key;
    if (nsid >= s->namespaces.slots_exports_hot.count)
      return false;
  ...
  case QUERY_NAMESPACE_TYPE: {
    uint32_t nsid = (uint32_t)key;
    if (nsid >= s->namespaces.slots_namespace_type_hot.count)
      return false;
  ...
  case QUERY_NAMESPACE_ITEMS: {
    uint32_t nsid = (uint32_t)key;
    if (nsid >= s->namespaces.slots_namespace_items_hot.count)
      return false;
  ...
  case QUERY_CHECK: {
    ...
    uint32_t nsid = (uint32_t)key;
    if (nsid >= s->namespaces.slots_check_hot.count)
      return false;
  ...
  case QUERY_TOP_LEVEL_ENTRY: {
    ...
    if (row >= s->top_level_entry.slots_hot.count)
      return false;
  ...
  case QUERY_DEF_IDENTITY: {
    ...
    if (!rowp)
      return false;
    row = (uint32_t)(uintptr_t)rowp;
    if (row >= s->def_identity.slots_hot.count)
      return false;
  ...
  case QUERY_RESOLVE_REF: {
    ...
    if (!rowp)
      return false;
    row = (uint32_t)(uintptr_t)rowp;
    if (row >= s->resolve_ref.slots_hot.count)
      return false;
  ...
  if (out_hot)
    *out_hot = (QuerySlotHot *)paged_get(hot_vec, row);
  if (out_cold)
    *out_cold = (QuerySlotCold *)paged_get(cold_vec, row);
  return true;
}
...
QuerySlotHot *db_engine_locate_slot(db_query_ctx *ctx, QueryKind kind,
  ...
  if (!db_engine_route_slot(ctx, kind, key, &hot, &cold))
    return NULL;
  return hot;
}
...
QueryBeginResult db_query_begin(db_query_ctx *ctx, QueryKind kind,
  ...
  assert(ok && slot && cold &&
         "db_query_begin: db_engine_route_slot returned NULL — slot kind not "
         "wired");
  (void)ok;
  ...
  case QUERY_ERROR: {
    ...
    if (db_engine_verify(ctx, slot)) {
      ...
      record_dep_on_parent(ctx, kind, key, slot->fingerprint, slot->durability,
                           1);
      query_stack_pop(ctx);
...
void db_query_slot_alloc(db_query_ctx *ctx, QueryKind kind, uint64_t key) {
  ...
  case QUERY_DEF_IDENTITY: {
    ...
    hashmap_put_or_die(&s->def_by_identity, key, (void *)(uintptr_t)row,
                       "engine: def_identity slot alloc");
    return;
  }
  case QUERY_RESOLVE_REF: {
...
void db_query_note_input_durability(db_query_ctx *ctx, Durability dur) {
  struct db *s = db_(ctx);
  assert(dur < DUR_COUNT &&
         "db_query_note_input_durability: durability out of range");
  if (s->query_stack.count == 0)
    return;
  QueryFrame *top =
      (QueryFrame *)vec_get(&s->query_stack, s->query_stack.count - 1);
