...
//   - preamble is top_level_entry + raw green root (no decl_ast / TOP_LEVEL_INDEX
//     ensure / multi-file loop).
// Builtins (@import etc.) are DEFERRED to D3 (SK_BUILTIN_EXPR → IP_NONE + TODO).
...
extern SyntaxNodePtr     db_body_scope_lookup(db_query_ctx *ctx, DefId fn_def, SyntaxNode *use_node, StrId name);
...
static const uint32_t COMPTIME_NUMERIC_MASK = IP_BIT(COMPTIME_INT) | IP_BIT(COMPTIME_FLOAT);
static const uint32_t NUMERIC_MASK = CONCRETE_INT_MASK | CONCRETE_FLOAT_MASK | COMPTIME_NUMERIC_MASK;

static bool is_concrete_int(IpIndex t)   { return t.v < 32u && ((CONCRETE_INT_MASK >> t.v) & 1u); }
static bool is_concrete_float(IpIndex t) { return t.v < 32u && ((CONCRETE_FLOAT_MASK >> t.v) & 1u); }
static bool is_numeric(IpIndex t)        { return t.v < 32u && ((NUMERIC_MASK >> t.v) & 1u); }

static IpIndex unify_arith(IpIndex a, IpIndex b) {
    if (a.v == IP_NONE.v || b.v == IP_NONE.v) return IP_NONE;
    if (a.v == b.v) return a;
    ...
    if (a.v == IP_COMPTIME_INT_TYPE.v && (is_concrete_int(b) || is_concrete_float(b))) return b;
    if (b.v == IP_COMPTIME_INT_TYPE.v && (is_concrete_int(a) || is_concrete_float(a))) return a;
    if (a.v == IP_COMPTIME_FLOAT_TYPE.v && is_concrete_float(b)) return b;
    if (b.v == IP_COMPTIME_FLOAT_TYPE.v && is_concrete_float(a)) return a;
    return IP_NONE;
}

static const char *opkind_name(SyntaxKind k) {
    switch (k) {
    case SK_PLUS: return "+"; case SK_MINUS: return "-"; case SK_STAR: return "*";
    case SK_SLASH: return "/"; case SK_PERCENT: return "%"; case SK_STAR_STAR: return "**";
    case SK_EQ_EQ: return "=="; case SK_BANG_EQ: return "!="; case SK_LT: return "<";
    case SK_LE: return "<="; case SK_GT: return ">"; case SK_GE: return ">=";
    case SK_AMP_AMP: return "&&"; case SK_PIPE_PIPE: return "||"; case SK_AMP: return "&";
    case SK_PIPE: return "|"; case SK_CARET: return "^"; case SK_SHL: return "<<";
    case SK_SHR: return ">>"; case SK_BANG: return "!"; case SK_TILDE: return "~";
    case SK_QUESTION: return "?"; default: return "?";
    }
}
...
static bool can_coerce(struct db *s, IpIndex actual, IpIndex expected) {
    if (actual.v == IP_NONE.v || expected.v == IP_NONE.v) return true;
    if (actual.v == expected.v) return true;
    if (actual.v == IP_NORETURN_TYPE.v) return true;
        ...
        if (ip_tag(&s->intern, ak.ptr_type.elem) == IP_TAG_ARRAY_TYPE) {
            ...
            if (et == IP_TAG_SLICE_TYPE || et == IP_TAG_SLICE_CONST_TYPE) {
                ...
                if (arrk.array_type.elem.v == ek.slice_type.elem.v &&
                    (!a_const || et == IP_TAG_SLICE_CONST_TYPE)) return true;
            }
            if (et == IP_TAG_MANY_PTR_TYPE || et == IP_TAG_MANY_PTR_CONST_TYPE) {
                IpKey ek = ip_key(&s->intern, expected);
                if (arrk.array_type.elem.v == ek.many_ptr_type.elem.v &&
                    (!a_const || et == IP_TAG_MANY_PTR_CONST_TYPE)) return true;
            }
        }
    }
    // nil → ?T / ^T / [^]T / []T
    if (actual.v == IP_NIL_TYPE.v &&
        (et == IP_TAG_OPTIONAL_TYPE || et == IP_TAG_PTR_TYPE || et == IP_TAG_PTR_CONST_TYPE ||
         et == IP_TAG_MANY_PTR_TYPE || et == IP_TAG_MANY_PTR_CONST_TYPE ||
         et == IP_TAG_SLICE_TYPE || et == IP_TAG_SLICE_CONST_TYPE))
        return true;
    // Optional lift: T → ?T
    if (et == IP_TAG_OPTIONAL_TYPE) {
        IpKey ek = ip_key(&s->intern, expected);
        if (can_coerce(s, actual, ek.optional_type.elem)) return true;
    }
...
static IpIndex resolve_value_path(const SemaCtx *ctx, SyntaxNode *use_node, StrId name) {
    if (name.idx == 0)
        return IP_NONE;
    struct db *s = ctx->s;

    if (ctx->enclosing_fn.idx != DEF_ID_NONE.idx) {
        SyntaxNodePtr bind = db_body_scope_lookup(s, ctx->enclosing_fn, use_node, name);
        if (bind.kind != SYNTAX_KIND_NONE) {
            ...
            if (ctx->types) {
                ...
                if (v) return (IpIndex){.v = (uint32_t)(uintptr_t)v};
            }
    ...
    db_emit(s, DIAG_ERROR, span_of(ctx, use_node), "undefined identifier '%S'", name);
    return IP_NONE;
}
...
static SyntaxNode **collect_arg_nodes(struct db *s, SyntaxNode *arg_list, uint32_t *out_count) {
    *out_count = 0;
    if (!arg_list) return NULL;
    uint32_t total = syntax_node_num_children(arg_list);
    uint32_t n = 0;
    for (uint32_t i = 0; i < total; i++)
        if (green_node_child(syntax_node_green(arg_list), i).kind == GREEN_ELEM_NODE) n++;
    if (n == 0) return NULL;
    ...
    for (uint32_t i = 0; i < total && k < n; i++) {
        ...
        if (el.kind == SYNTAX_ELEM_NODE && el.node) out[k++] = el.node;
        else if (el.kind == SYNTAX_ELEM_TOKEN && el.token) syntax_token_release(el.token);
    }
    *out_count = k;
    return out;
}
static void release_arg_nodes(SyntaxNode **args, uint32_t count) {
    for (uint32_t i = 0; i < count; i++) if (args[i]) syntax_node_release(args[i]);
}
...
static void bind_parts(SyntaxNode *decl, SyntaxKind k, SyntaxNode **type_out, SyntaxNode **val_out) {
    *type_out = NULL; *val_out = NULL;
    if (k == SK_CONST_DECL) { ConstDef c; if (ConstDef_cast(decl, &c)) { *type_out = ConstDef_type(&c); *val_out = ConstDef_value(&c); } }
    else                    { VarDef v;  if (VarDef_cast(decl, &v))    { *type_out = VarDef_type(&v);  *val_out = VarDef_value(&v);  } }
}
...
static IpIndex type_let_bind(const SemaCtx *ctx, SyntaxNode *decl, SyntaxKind k) {
    SyntaxNode *type_node, *value_node;
    bind_parts(decl, k, &type_node, &value_node);
    IpIndex bt = IP_NONE;
    if (type_node) {
        bt = resolve_type_expr(ctx, type_node);
        if (value_node && bt.v != IP_NONE.v) (void)check_expr(ctx, value_node, bt);
    } else if (value_node) {
...
static bool pattern_is_wildcard(SyntaxNode *p) {
    if (syntax_node_kind(p) != SK_LITERAL_EXPR) return false;
    Literal lit;
...
static IpIndex infer_switch(const SemaCtx *ctx, SyntaxNode *node, IpIndex expected) {
    struct db *s = ctx->s;
    SwitchExpr sw;
    if (!SwitchExpr_cast(node, &sw)) return IP_NONE;
    ...
    if (arms) {
        ...
        for (uint32_t i = 0; i < n; i++) {
            ...
            if (ael.kind != SYNTAX_ELEM_NODE || !ael.node) {
                if (ael.kind == SYNTAX_ELEM_TOKEN && ael.token) syntax_token_release(ael.token);
                continue;
            }
            SyntaxNode *arm = ael.node;
            if (syntax_node_kind(arm) != SK_SWITCH_ARM) { syntax_node_release(arm); continue; }
            ...
            if (prev) {  // the body
                if (check_mode) {
                    ...
                    if (!result_set) { result = bt; result_set = true; }
                    else if (bt.v != IP_NONE.v && result.v != IP_NONE.v && bt.v != result.v) {
                        IpIndex u = unify_arith(result, bt);
                        if (u.v != IP_NONE.v) result = u;
                        else db_emit(s, DIAG_ERROR, span_of(ctx, prev),
...
static IpIndex type_of_expr_impl(const SemaCtx *ctx, SyntaxNode *node) {
    ...
    case SK_LITERAL_EXPR: {
        Literal lit;
        if (!Literal_cast(node, &lit)) return IP_NONE;
        return type_from_lit_token(Literal_kind(&lit));
    }

    case SK_REF_EXPR: {
        RefExpr r;
        if (!RefExpr_cast(node, &r)) return IP_NONE;
    ...
    case SK_PATH_EXPR: {
        ...
        for (uint32_t i = 0; i < count; i++) {
            ...
            if (g.kind == GREEN_ELEM_TOKEN && green_token_kind(g.token) == SK_IDENT)
                last = pool_intern(&s->strings, green_token_text(g.token), green_token_text_len(g.token));
        }
    ...
    case SK_PAREN_EXPR: {
        ParenExpr pe;
        if (!ParenExpr_cast(node, &pe)) return IP_NONE;
    ...
    case SK_BIN_EXPR: {
        BinExpr be;
        if (!BinExpr_cast(node, &be)) return IP_NONE;
        ...
        if (rhs) syntax_node_release(rhs);
        if (lt.v == IP_NONE.v || rt.v == IP_NONE.v) return IP_NONE;
        ...
        if (is_arith) {
            ...
            if (u.v == IP_NONE.v || !is_numeric(u)) {
                db_emit(s, DIAG_ERROR, span_of(ctx, node),
                        "cannot apply '%s' to operands of type %T and %T", opkind_name(opk), lt, rt);
                return IP_NONE;
            }
            return u;
        }
        ...
        if (is_logical) {
            if (lt.v != IP_BOOL_TYPE.v || rt.v != IP_BOOL_TYPE.v) {
                db_emit(s, DIAG_ERROR, span_of(ctx, node),
                        ...
                        opkind_name(opk), (lt.v != IP_BOOL_TYPE.v) ? lt : rt);
                return IP_NONE;
            }
            return IP_BOOL_TYPE;
        }
        if (is_bitop) {
            ...
            if (u.v == IP_NONE.v || !lt_ok || !rt_ok) {
                db_emit(s, DIAG_ERROR, span_of(ctx, node),
                        "bitwise '%s' requires integer operands, got %T and %T", opkind_name(opk), lt, rt);
                return IP_NONE;
            }
            return u;
        }
        ...
        db_emit(s, DIAG_ERROR, span_of(ctx, node),
                "binary operator '%s' not yet supported in type inference", opkind_name(opk));
        return IP_NONE;
    }

    case SK_PREFIX_EXPR: {
        PrefixExpr pe;
        if (!PrefixExpr_cast(node, &pe)) return IP_NONE;
        SyntaxKind opk = PrefixExpr_op_kind(&pe);
        SyntaxNode *operand = PrefixExpr_operand(&pe);
        if (!operand) return IP_NONE;
        ...
        if (opk == SK_AMP) {
            ...
            if (!is_lvalue && ck == SK_POSTFIX_EXPR) {
                PostfixExpr in;
                if (PostfixExpr_cast(operand, &in) && PostfixExpr_op_kind(&in) == SK_CARET)
                    is_lvalue = true;
            }
            if (!is_lvalue) {
                db_emit(s, DIAG_ERROR, span_of(ctx, node),
                        "address-of '&' requires an l-value (variable, field, index, or deref)");
                syntax_node_release(operand);
                return IP_NONE;
            }
            IpIndex t = type_of_expr(ctx, operand);
            syntax_node_release(operand);
            if (t.v == IP_NONE.v) return IP_NONE;
            return ip_get(&s->intern, (IpKey){.kind = IPK_PTR_TYPE,
        ...
        if (t.v == IP_NONE.v) return IP_NONE;
        if (opk == SK_MINUS) {
            if (!is_numeric(t)) { db_emit(s, DIAG_ERROR, span_of(ctx, node), "unary '-' requires numeric operand, got %T", t); return IP_NONE; }
            return t;
        }
        if (opk == SK_TILDE) {
            if (t.v != IP_COMPTIME_INT_TYPE.v && !is_concrete_int(t)) { db_emit(s, DIAG_ERROR, span_of(ctx, node), "unary '~' requires integer operand, got %T", t); return IP_NONE; }
            return t;
        }
        if (opk == SK_BANG) {
            if (t.v != IP_BOOL_TYPE.v) { db_emit(s, DIAG_ERROR, span_of(ctx, node), "unary '!' requires bool, got %T", t); return IP_NONE; }
            return IP_BOOL_TYPE;
        }
        db_emit(s, DIAG_ERROR, span_of(ctx, node),
                "prefix operator '%s' not yet supported in type inference", opkind_name(opk));
        return IP_NONE;
    }

    case SK_POSTFIX_EXPR: {
        PostfixExpr po;
        if (!PostfixExpr_cast(node, &po)) return IP_NONE;
        ...
        if (opk == SK_CARET) {
            ...
            if (tag != IP_TAG_PTR_TYPE && tag != IP_TAG_PTR_CONST_TYPE) {
                db_emit(s, DIAG_ERROR, span_of(ctx, node), "cannot dereference non-pointer type %T", t);
                return IP_NONE;
            }
        ...
        if (opk == SK_QUESTION) {
            if (ip_tag(&s->intern, t) != IP_TAG_OPTIONAL_TYPE) {
                db_emit(s, DIAG_ERROR, span_of(ctx, node), "'.?' requires optional type, got %T", t);
                return IP_NONE;
            }
    ...
    case SK_CALL_EXPR: {
        CallExpr ce;
        if (!CallExpr_cast(node, &ce)) return IP_NONE;
        ...
        if (ip_tag(&s->intern, callee_ty) != IP_TAG_FN_TYPE) {
            db_emit(s, DIAG_ERROR, span_of(ctx, callee ? callee : node), "value of type %T is not callable", callee_ty);
    ...
    case SK_FIELD_EXPR: {
        FieldExpr fe;
        if (!FieldExpr_cast(node, &fe)) return IP_NONE;
        ...
        if (recv.v == IP_NONE.v || fname.idx == 0) return IP_NONE;

        IpTag tag = ip_tag(&s->intern, recv);
        if (tag == IP_TAG_PTR_TYPE || tag == IP_TAG_PTR_CONST_TYPE) {  // auto-deref single ptr
        ...
        case IP_TAG_ENUM_TYPE: {
            ...
            for (uint32_t i = 0; i < nv; i++)
                if (vs[i].name.idx == fname.idx) return recv;
            db_emit(s, DIAG_ERROR, fspan, "no variant '%S' in %T", fname, recv);
            return IP_NONE;
        }
        case IP_TAG_SLICE_TYPE:
        case IP_TAG_SLICE_CONST_TYPE: {
            if (fname.idx == s->names.LEN.idx) return IP_USIZE_TYPE;
            if (fname.idx == s->names.PTR.idx) {
                IpKey sk = ip_key(&s->intern, recv);
                return ip_get(&s->intern, (IpKey){.kind = IPK_MANY_PTR_TYPE,
                    .many_ptr_type = {.elem = sk.slice_type.elem, .is_const = (tag == IP_TAG_SLICE_CONST_TYPE)}});
            }
            db_emit(s, DIAG_ERROR, fspan, "no field '%S' on slice (only '.len' and '.ptr')", fname);
            return IP_NONE;
        }
        case IP_TAG_ARRAY_TYPE:
            if (fname.idx == s->names.LEN.idx) return IP_USIZE_TYPE;
            db_emit(s, DIAG_ERROR, fspan, "no field '%S' on array (only '.len')", fname);
            return IP_NONE;
        case IP_TAG_NAMESPACE_TYPE: {
            ...
            for (uint32_t i = 0; i < n; i++) {
                ...
                if (m.name.idx == fname.idx) return db_query_type_of_def(s, m.def);
            }
            db_emit(s, DIAG_ERROR, fspan, "no member '%S' in %T", fname, recv);
            return IP_NONE;
        }
        default:
            db_emit(s, DIAG_ERROR, fspan, "field access on non-aggregate type %T", recv);
            return IP_NONE;
        }
    }
    ...
    case SK_INDEX_EXPR: {
        IndexExpr ie;
        if (!IndexExpr_cast(node, &ie)) return IP_NONE;
        SyntaxNode *base = IndexExpr_base(&ie), *idx = IndexExpr_index(&ie);
        IpIndex obj = base ? type_of_expr(ctx, base) : IP_NONE;
        if (idx) (void)type_of_expr(ctx, idx);
        DiagAnchor bspan = span_of(ctx, base ? base : node);
        if (base) syntax_node_release(base);
        if (idx) syntax_node_release(idx);
        if (obj.v == IP_NONE.v) return IP_NONE;
        IpKey key = ip_key(&s->intern, obj);
        switch (ip_tag(&s->intern, obj)) {
        case IP_TAG_ARRAY_TYPE: return key.array_type.elem;
        case IP_TAG_SLICE_TYPE:
        case IP_TAG_SLICE_CONST_TYPE: return key.slice_type.elem;
        case IP_TAG_MANY_PTR_TYPE:
        case IP_TAG_MANY_PTR_CONST_TYPE: return key.many_ptr_type.elem;
        default:
    ...
    case SK_SLICE_EXPR: {
        SliceExpr se;
        if (!SliceExpr_cast(node, &se)) return IP_NONE;
        SyntaxNode *base = SliceExpr_base(&se), *lo = SliceExpr_lo(&se), *hi = SliceExpr_hi(&se);
        IpIndex obj = base ? type_of_expr(ctx, base) : IP_NONE;
        if (lo) (void)type_of_expr(ctx, lo);
        if (hi) (void)type_of_expr(ctx, hi);
        DiagAnchor bspan = span_of(ctx, base ? base : node);
        if (base) syntax_node_release(base);
        ...
        bool is_const = false;
        switch (ip_tag(&s->intern, obj)) {
        case IP_TAG_ARRAY_TYPE: elem = key.array_type.elem; break;
        case IP_TAG_SLICE_TYPE: elem = key.slice_type.elem; break;
        case IP_TAG_SLICE_CONST_TYPE: elem = key.slice_type.elem; is_const = true; break;
        case IP_TAG_MANY_PTR_TYPE: elem = key.many_ptr_type.elem; break;
        case IP_TAG_MANY_PTR_CONST_TYPE: elem = key.many_ptr_type.elem; is_const = true; break;
        default:
            db_emit(s, DIAG_ERROR, bspan, "value of type %T is not sliceable", obj);
            return IP_NONE;
        }
        return ip_get(&s->intern, (IpKey){.kind = IPK_SLICE_TYPE,
                                          .slice_type = {.elem = elem, .is_const = is_const}});
    }

    case SK_RETURN_STMT: {
        ReturnStmt rs;
        if (!ReturnStmt_cast(node, &rs)) return IP_NORETURN_TYPE;
    ...
    case SK_IF_EXPR: {
        IfExpr ie;
        if (!IfExpr_cast(node, &ie)) return IP_NONE;
    ...
    case SK_LOOP_EXPR: {
        LoopExpr le;
        if (!LoopExpr_cast(node, &le)) return IP_NONE;
        SyntaxNode *body = LoopExpr_body(&le);
        if (body) { (void)type_of_expr(ctx, body); syntax_node_release(body); }
        return IP_VOID_TYPE;
    }
    ...
    case SK_ASSIGN_EXPR: {
        AssignExpr ae;
        if (!AssignExpr_cast(node, &ae)) return IP_NONE;
        SyntaxNode *lhs = AssignExpr_lhs(&ae), *rhs = AssignExpr_rhs(&ae);
        IpIndex lt = lhs ? type_of_expr(ctx, lhs) : IP_NONE;
        if (rhs) (void)check_expr(ctx, rhs, lt);
        if (lhs) syntax_node_release(lhs);
        if (rhs) syntax_node_release(rhs);
        return IP_VOID_TYPE;
    }
    ...
    case SK_EXPR_STMT: {
        ...
        for (uint32_t i = 0; i < total; i++) {
            ...
            if (el.kind == SYNTAX_ELEM_NODE && el.node) { t = type_of_expr(ctx, el.node); syntax_node_release(el.node); }
            else if (el.kind == SYNTAX_ELEM_TOKEN && el.token) syntax_token_release(el.token);
        }
    ...
    case SK_LAMBDA_EXPR: {
        LambdaExpr lam;
        if (!LambdaExpr_cast(node, &lam)) return IP_NONE;
...
bool check_expr(const SemaCtx *ctx, SyntaxNode *node, IpIndex expected) {
    ...
    if (expected.v != IP_NONE.v) {
        ...
        if (k == SK_ENUM_REF_EXPR && ip_tag(&s->intern, expected) == IP_TAG_ENUM_TYPE) {
            EnumRefExpr er;
            if (EnumRefExpr_cast(node, &er)) {
                ...
                const EnumVariantEntry *vs = db_enum_variants(s, d, &nv);
                for (uint32_t i = 0; i < nv; i++)
                    if (vs[i].name.idx == vname.idx) return true;
                db_emit(s, DIAG_ERROR, span_of(ctx, node), "no such variant in %T", expected);
                return false;
            }
        }
        ...
        if (k == SK_SWITCH_EXPR) {
            (void)infer_switch(ctx, node, expected);  // checks each arm body vs expected
            return true;
        }
            ...
            BlockStmt bs = {.syntax = node};  // kind validated above
            {
                ...
                if (!stmts) {
                    if (!can_coerce(s, IP_VOID_TYPE, expected)) {
                        db_emit(s, DIAG_ERROR, span_of(ctx, node), "empty block returns void; expected %T", expected);
                        return false;
                    }
                    return true;
                }
                ...
                for (uint32_t i = 0; i < total; i++) {
                    SyntaxElement el = syntax_node_child_or_token(stmts, i);
                    if (el.kind == SYNTAX_ELEM_NODE && el.node) { node_count++; syntax_node_release(el.node); }
                    else if (el.kind == SYNTAX_ELEM_TOKEN && el.token) syntax_token_release(el.token);
                }
                if (node_count == 0) {
                    syntax_node_release(stmts);
                ...
                for (uint32_t i = 0; i < total; i++) {
                    ...
                    if (seen == node_count - 1) {
                        if (!check_expr(ctx, stmt, expected)) ok = false;
                    } else {
                        IpIndex t = type_of_expr(ctx, stmt);
                        if (t.v != IP_NONE.v && t.v != IP_VOID_TYPE.v && t.v != IP_NORETURN_TYPE.v &&
                            !kind_is_discard_construct(syntax_node_kind(stmt)))
                            db_emit(s, DIAG_WARNING, span_of(ctx, stmt), "unused value of type %T", t);
                    }
                    seen++;
        ...
        if (k == SK_IF_EXPR) {
            ...
            if (IfExpr_cast(node, &ie)) {
                ...
                bool ok = true;
                handle_if_cond(ctx, cond);
                if (then_b && !check_expr(ctx, then_b, expected)) ok = false;
                if (else_b && !check_expr(ctx, else_b, expected)) ok = false;
                if (cond) syntax_node_release(cond);
                if (then_b) syntax_node_release(then_b);
                if (else_b) syntax_node_release(else_b);
                return ok;
        ...
        if (k == SK_BIN_EXPR) {
            ...
            if (BinExpr_cast(node, &be)) {
                SyntaxKind opk = BinExpr_op_kind(&be);
                bool propagate = (opk == SK_PLUS || opk == SK_MINUS || opk == SK_STAR ||
                                  opk == SK_SLASH || opk == SK_PERCENT || opk == SK_STAR_STAR ||
                                  opk == SK_AMP || opk == SK_PIPE || opk == SK_CARET);
                if (propagate) {
                    SyntaxNode *lhs = BinExpr_lhs(&be), *rhs = BinExpr_rhs(&be);
                    bool ok = true;
                    if (lhs && !check_expr(ctx, lhs, expected)) ok = false;
                    if (rhs && !check_expr(ctx, rhs, expected)) ok = false;
                    if (lhs) syntax_node_release(lhs);
                    if (rhs) syntax_node_release(rhs);
        ...
        if (k == SK_LITERAL_EXPR || k == SK_REF_EXPR || k == SK_PATH_EXPR) {
            ...
            bool actual_comptime = (actual.v == IP_COMPTIME_INT_TYPE.v || actual.v == IP_COMPTIME_FLOAT_TYPE.v);
            bool expected_comptime = (expected.v == IP_COMPTIME_INT_TYPE.v || expected.v == IP_COMPTIME_FLOAT_TYPE.v);
            if (actual_comptime && !expected_comptime && can_coerce(s, actual, expected)) {
                node_type_builder_push(ctx, node, expected);
                return true;
            }
            if (can_coerce(s, actual, expected)) return true;
            db_emit(s, DIAG_ERROR, span_of(ctx, node), "expected %T, got %T", expected, actual);
            return false;
        }
    }
...
NodeTypesRange db_query_infer_body(db_query_ctx *ctx, DefId def) {
    ...
    const FnSignature *sig = db_query_fn_signature(ctx, def);   // dep: declared return
    ...
    if (e.node_ptr.kind != SYNTAX_KIND_NONE) {
        ...
        struct GreenNode *groot = *(struct GreenNode **)vec_get(&s->files.green_roots, local);
        if (groot) {
            ...
            if (wrapper) {
                ...
                if (wk == SK_CONST_DECL) { ConstDef cd; if (ConstDef_cast(wrapper, &cd)) val = ConstDef_value(&cd); }
                else if (wk == SK_VAR_DECL) { VarDef vd; if (VarDef_cast(wrapper, &vd)) val = VarDef_value(&vd); }
                if (val) {
                    if (syntax_node_kind(val) == SK_LAMBDA_EXPR) lambda_node = val;
                    else syntax_node_release(val);
                }
    ...
    if (lambda_node) {
        ...
        if (LambdaExpr_cast(lambda_node, &lam)) {
            ...
            SemaCtx walk = {.s = s, .file_green_root = NULL, .nsid = nsid,
                            .enclosing_fn = def, .file_local = e.file, .types = &b};
            bool sig_is_fn = (sigty.v != IP_NONE.v && ip_tag(&s->intern, sigty) == IP_TAG_FN_TYPE);
            ...
            IpIndex expected_ret = sig_is_fn ? ip_key(&s->intern, sigty).fn_type.ret : IP_NONE;
            if (body) { (void)check_expr(&walk, body, expected_ret); syntax_node_release(body); }
