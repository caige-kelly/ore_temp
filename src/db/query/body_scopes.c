...
static uint32_t scope_push(BSBuilder *b, uint32_t parent, SyntaxNode *block_node) {
...
static void bind_push(BSBuilder *b, uint32_t scope_id, StrId name,
    ...
    SyntaxNodePtr bs = bind_node ? syntax_node_ptr_new(bind_node)
                                 : (SyntaxNodePtr){0};
...
static void walk(SyntaxNode *node, BSBuilder *b, uint32_t current_scope) {
    ...
    if (k == SK_BLOCK_STMT) {
        BlockStmt bs;
        if (!BlockStmt_cast(node, &bs)) { walk_children(node, b, current_scope); return; }
        uint32_t child = scope_push(b, current_scope, node);
        SyntaxNode *stmts = BlockStmt_stmts(&bs);
        if (stmts) { walk_children(stmts, b, child); syntax_node_release(stmts); }
        return;
    }
    ...
    if (k == SK_CONST_DECL || k == SK_VAR_DECL) {
        ...
        if (type_node) { walk(type_node, b, current_scope); syntax_node_release(type_node); }
        if (value_node) { walk(value_node, b, current_scope); syntax_node_release(value_node); }
    ...
    if (k == SK_IF_EXPR) {
        IfExpr ie;
        if (!IfExpr_cast(node, &ie)) { walk_children(node, b, current_scope); return; }
        ...
        if (is_if_let) {
            ...
            if (ck == SK_CONST_DECL) {
                ConstDef cd;
                if (ConstDef_cast(cond, &cd)) { name_tok = ConstDef_name(&cd); rhs = ConstDef_value(&cd); }
            } else {
                VarDef vd;
                if (VarDef_cast(cond, &vd)) { name_tok = VarDef_name(&vd); rhs = VarDef_value(&vd); }
            }
            StrId name = intern_tok(s, name_tok);
            if (name_tok) syntax_token_release(name_tok);
            // RHS lives in the PARENT scope (can't see its own bind).
            if (rhs) walk(rhs, b, current_scope);
    ...
    if (k == SK_LOOP_EXPR) {
        LoopExpr le;
        if (!LoopExpr_cast(node, &le)) { walk_children(node, b, current_scope); return; }
        uint32_t loop_scope = scope_push(b, current_scope, node);
        SyntaxNode *body = LoopExpr_body(&le);
        if (body) { walk(body, b, loop_scope); syntax_node_release(body); }
        return;
    }
    ...
    if (k == SK_SWITCH_ARM) {
        SwitchArm arm;
        if (!SwitchArm_cast(node, &arm)) { walk_children(node, b, current_scope); return; }
        uint32_t arm_scope = scope_push(b, current_scope, node);
        SyntaxNode *pat = SwitchArm_pattern(&arm);
        SyntaxNode *body = SwitchArm_body(&arm);
        if (pat) { walk(pat, b, arm_scope); syntax_node_release(pat); }
        if (body) { walk(body, b, arm_scope); syntax_node_release(body); }
        return;
    }
...
const FnBody *db_query_body_scopes(db_query_ctx *ctx, DefId def) {
    ...
    if (e.node_ptr.kind != SYNTAX_KIND_NONE) {
        ...
        if (groot) {
            ...
            if (wrapper) {
                ...
                if (val) {
                    if (syntax_node_kind(val) == SK_LAMBDA_EXPR) lambda_node = val;  // keep
    ...
    if (LambdaExpr_cast(lambda_node, &lam)) {
        ...
        if (ret)  { walk(ret,  &b, root); syntax_node_release(ret); }
        if (body) { walk(body, &b, root); syntax_node_release(body); }
    }
