...
// Mark-and-copy compaction for the shared salsa pools. Each compactor
// walks the per-kind columns that own ranges into its pool, builds a
// sorted (old_off, len, new_off) remap table, allocates a fresh pool,
// copies live ranges into it, rewrites every column entry with the
// new offsets, then swaps in the new pool and frees the old.
//
